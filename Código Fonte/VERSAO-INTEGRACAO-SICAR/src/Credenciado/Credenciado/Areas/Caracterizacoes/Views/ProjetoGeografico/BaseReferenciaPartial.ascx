			...
			<table class="dataGridTable gridArquivosVetoriais">
				...
				<tbody>
					<% foreach (var item in Model.ArquivosVetoriais) { %>
						<tr>
							...
							<td>
								...
								<input type="hidden" class="hdnProcessamentoFilaId" value="<%: item.ProcessamentoId %>"/>
								<input type="hidden" class="hdnArquivoEnviadoFilaTipo" value="<%: item.FilaTipo %>"/>
								<input type="hidden" class="hdnArquivoEnviadoSituacaoId" value="<%= item.Situacao %>" />
								<input type="hidden" class="hdnProcessamentoEtapa" value="<%= item.Etapa %>" />
								<input type="hidden" class="hdnProcessamentoMecanismo" value="<%= item.Mecanismo %>" />

								<span class="StatusFinalizado <%= Model.SituacaoProjeto == (int)eProjetoGeograficoSituacao.Finalizado || Model.IsVisualizar ? "hide" : "" %>">
									<span class="btnGerar <%= item.MostrarGerar ? "" : "hide" %>"><input type="button"  class="icone opcoes " title="Gerar"/></span>
								</span>

								<span class="btnDownload <%= item.MostrarBaixar ? "" : "hide" %>"><input type="button" class="icone download" title="Baixar"/></span>
								
								<% if (!Model.IsVisualizar){ %>
								<span class="StatusFinalizado <%= Model.SituacaoProjeto == (int)eProjetoGeograficoSituacao.Finalizado ? "hide" : "" %>">
									<span class="btnRegerar <%= item.MostrarRegerar ? "" : "hide" %>"><input type="button"  class="icone refresh" title="Regerar"/></span>
								</span>
								<%} %>
							</td>
						</tr>
					...
					<tr class="linhaModelo">
						...
						<td>
							<span class="btnDownloadModelo"><input type="button"  class="icone download" title="Baixar"/></span>
						</td>
					</tr>
				</tbody>
			</table>
				...
				<tbody>
					<tr class="trTempletaArqVetorial">
						...
						<td>
							...
							<input type="hidden" class="hdnArquivoTipo" />
							<input type="hidden" class="hdnProcessamentoFilaId" value=""/>
							<input type="hidden" class="hdnArquivoEnviadoFilaTipo" value=""/>
							<input type="hidden" class="hdnArquivoEnviadoSituacaoId" value=""/>
							<input type="hidden" class="hdnProcessamentoEtapa" value=""/>
							<input type="hidden" class="hdnProcessamentoMecanismo" value=""/>
					...
					<% foreach (var item in Model.ArquivosOrtoFotoMosaico) { %>
						<tr>
							...
							<td>
								...
								<span class="btnDownload"><input type="button" class="icone download"/></span>
							</td>
						</tr>
					<% } %>
				</tbody>
