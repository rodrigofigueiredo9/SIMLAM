...
namespace Tecnomapas.EtramiteX.Credenciado.Areas.Caracterizacoes.ViewModels.VMProjetoGeografico
{
	public class ProjetoGeograficoVM
	{
		...
		public bool MostrarAvancar { get; set; }

		public bool RetornarVisualizar { get; set; }
		...
		private ImportadorShapeVM _importador = new ImportadorShapeVM();
		public ImportadorShapeVM Importador
		{
		...
		public List<ArquivoProcessamentoVM> ArquivosProcessados
		{
			get
			{
				if (Importador != null && Importador.ArquivosProcessados.Count > 0)
				{
					return Importador.ArquivosProcessados;
				}
		...
		public String SituacoesProcessamento
		{
			get
			{
				return ViewModelHelper.Json(new
				{
					...
					@AguardandoValidacao = (int)eProjetoGeograficoSituacaoProcessamento.AguardandoValidacao,
					@Cancelada = (int)eProjetoGeograficoSituacaoProcessamento.Cancelada,
					...
					@ErroValidacao = (int)eProjetoGeograficoSituacaoProcessamento.ErroValidacao,
					@ExecutantoValidacao = (int)eProjetoGeograficoSituacaoProcessamento.ExecutantoValidacao,
					...
					@Reprovado = (int)eProjetoGeograficoSituacaoProcessamento.Reprovado
				});
			}
		}

		public String EtapasProcessamento
		{
			get
			{
				return ViewModelHelper.Json(new
				{
					@Validacao = (int)eFilaEtapaGeo.Validacao,
					@Processamento = (int)eFilaEtapaGeo.Processamento,
					@GeracaoDePDF = (int)eFilaEtapaGeo.GeracaoDePDF
				});
			}
		}

		public String MecanismosProcessamento
		{
			get
			{
				return ViewModelHelper.Json(new
				{
					@Desenhador = (int)eProjetoGeograficoMecanismo.Desenhador,
					@ImportadorShape = (int)eProjetoGeograficoMecanismo.ImportadorShapes
				});
			}
		}

		public String ArquivoTipo
		{
			get
			{
				return ViewModelHelper.Json(new
				{
					@ArquivoEnviado = (int)eProjetoGeograficoArquivoTipo.ArquivoEnviado,
					@ArquivoProcessadoSoftwareGIS = (int)eProjetoGeograficoArquivoTipo.ArquivoProcessadoSoftwareGIS,
					@ArquivoProcessadoTrackMaker = (int)eProjetoGeograficoArquivoTipo.ArquivoProcessadoTrackMaker,
					@Croqui = (int)eProjetoGeograficoArquivoTipo.Croqui,
					@CroquiFinal = (int)eProjetoGeograficoArquivoTipo.CroquiFinal,
					@DadosGEOBASES = (int)eProjetoGeograficoArquivoTipo.DadosGEOBASES,
					@DadosIDAF = (int)eProjetoGeograficoArquivoTipo.DadosIDAF,
					@RelatorioImportacao = (int)eProjetoGeograficoArquivoTipo.RelatorioImportacao
				});
			}
		}
		...
		{
			Importador.SituacaoProjeto = Projeto.SituacaoId;
			...
			if (arquivos != null && arquivos.Count > 0)
			{
				foreach (ArquivoProjeto item in arquivos)
				{
					...
					if (item.Processamento.Situacao <= 0)
					{
						item.Processamento.Situacao = 9;
						item.Processamento.SituacaoTexto = "Processado";
					}
			...
			if (arquivos != null && arquivos.Count > 0)
			{
				foreach (ArquivoProjeto item in arquivos)
				{
					...
					if (Projeto.MecanismoElaboracaoId == (int)eProjetoGeograficoMecanismo.ImportadorShapes)
					{
						...
						Importador.ArquivosProcessados.Add(arqProcessamento);
					}
					else
					{
