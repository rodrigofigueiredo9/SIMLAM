		...
		<% foreach (var item in Model.Resultados) { %>
			<tr>
				...
				<td>
					...
					<button type="button" class="icone historico btnHistoricoTramitacao" title="Histórico de tramitação" ></button>
					<%if (item.PossuiPendencia){ %><button type="button" class="icone pendencias_cor btnNotificacaoPendencia" title="Notificações de Pendências" ></button><%} %>
				</td>
			</tr>
		<% } %>
		</tbody>
