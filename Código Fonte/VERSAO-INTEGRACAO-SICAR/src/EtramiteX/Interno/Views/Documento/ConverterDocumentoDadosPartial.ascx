...
<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<ConverterDocumentoVM>" %>
...
<div class="divModalProcesso divVisualizarProcesso">
	...
	<div class="block box">
		...
		<div class="block">
			...
			<div class="coluna40 prepend2">
				<label>Tipo *</label>
				<%= Html.DropDownList("ddlProcessoTipos", Model.ProcessoTipos, new { @class = "text ddlProcessoTipos disabled", @disabled = "disabled" })%>
			</div>
		</div>
		...
		<div class="block">
			<div class="block divPossuiSEP">
				<div class="coluna100">
					...
					<label><%= Html.RadioButton("Processo.PossuiSEP", 2, new { @class = "radio rdbProcessoPossuiSEP"})%>Não</label>
				</div>
			</div>
			<div class="block divAutuacao hide">
				<div class="coluna18">
					<label>Nº de autuação (SEP) *</label>
					<%= Html.TextBox("Processo.NumeroAutuacao", "", new { @class = "text txtNumeroAutuacao" })%>
				</div>

				<div class="coluna22 prepend2">
					<label>Data de autuação (SEP) *</label>
					<%= Html.TextBox("Processo.DataAutuacao", "", new { @class = "text maskData txtDataAutuacao" })%>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="divRequerimento">
		<% Html.RenderPartial("ConverterDocumentoRequerimentoVisualizar", Model.RequerimentoVM); %>
	</div>
	<% } %>
