...
<div class="dataGrid <%= ((Model.Paginacao.QuantidadeRegistros > 0) ? string.Empty : "hide") %> ">
	...
	<table class="dataGridTable ordenavel" width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<tbody>
			<% foreach (var grupo in Model.Resultados) { %>
			<tr>
				<td class="atividadeEmpTexto" title="<%= Html.Encode(grupo.NomeGrupo) %>"><%= Html.Encode(grupo.NomeGrupo)%></td>
				<td>
					...
					<input type="hidden" class="itemId" value="<%= Html.Encode(grupo.Id) %>" />
				</td>
			</tr>
			<% } %>
		</tbody>
