		...
		<% foreach (var item in Model.Resultados) { %>
			<tr>
				...
				<td>
					...
					<%if (Model.PodeRegerarChave && item.Situacao != (int)eCredenciadoSituacao.Cadastrado) { %><input type="button" title="Regerar Chave" class="icone chaveAcesso btnRegerar" /><%}%>
					<%if (Model.PodeAlterarSituacao) {%>
						<% if (item.PodeBloquear) { %>
							<input type="button" name="alterar situação" title="Alterar situação" class="icone altStatus btnAltStatus" />
						<% }%>
					<% }%>

					<% if (Model.PodeHabilitar) { %> <input type="button" name="Habilitar emissão" title="Habilitar emissão" class="icone editarProcesso btnHabilitar" /> <% }%>
				</td>
			</tr>
		<% } %>
		</tbody>
