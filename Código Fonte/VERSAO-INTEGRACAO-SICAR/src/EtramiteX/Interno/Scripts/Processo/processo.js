...
Processo = {
	settings: {
		...
		urls: {
			...
			desassociarFiscalizacaoValidar: '',
			obterFiscalizacao: '',
	...
	setarBotoes: function (container) {
		...
		container.delegate('.rdbProcessoPossuiSEP', 'change', Processo.gerenciarSEP);
		container.delegate('.btnBuscarFiscalizacao', 'click', Processo.onAssociarFiscalizacao);
		container.delegate('.btnLimparFiscalizacao', 'click', Processo.onLimparFiscalizacao);
		container.delegate('.btnVisualizarFiscalizacao', 'click', Processo.onAbrirVisualizarFiscalizacao);
	},

	gerenciarSEP: function () {

		var container = Processo.settings.container;
		var opcao = $('.rdbProcessoPossuiSEP:checked', container).val();
		$('.divAutuacao', container).removeClass('hide');

		if (opcao == 1) {
			$('.txtNumeroAutuacao', container).removeClass('disabled').removeAttr("disabled");
			$('.txtDataAutuacao', container).removeClass('disabled').removeAttr("disabled");
			$('.btnProcessoAutuar', container).addClass('hide');

			$('.divAutuacao', container).removeClass('hide');
		} else {
			/*$('.txtNumeroAutuacao', container).addClass('disabled').attr("disabled", "disabled");
			$('.txtDataAutuacao', container).addClass('disabled').attr("disabled", "disabled");
			$('.btnProcessoAutuar', container).removeClass('hide');*/

			$('.divAutuacao', container).addClass('hide');
		}
	...
	configurarAssociarMultiplo: function () {
		...
		if (!containerLink.hasClass('hide')) {
			$('.conteudoResponsaveis', Processo.settings.container).find('.btnAsmEditar').hide();
		}
	},
	...
	changeProcessoTipo: function () {
		...
		$('.containerFiscalizacao', Processo.settings.container).toggleClass('hide', (configuracao == null || (!configuracao.PossuiFiscalizacao && !configuracao.FiscalizacaoObrigatorio)));
		...
		$('.containerFiscalizacao', Processo.settings.container).toggleClass('apagar', (configuracao == null || (!configuracao.PossuiFiscalizacao && !configuracao.FiscalizacaoObrigatorio)));

		$('.labelInteressado', Processo.settings.container).text(configuracao.LabelInteressado);
		...
		$('.btnAssociarInteressado', Processo.settings.container).toggleClass('hide', configuracao.FiscalizacaoObrigatorio);

		Processo.onLimparFiscalizacaoEmCadastro();
	},
	...
	onAssociarFiscalizacao: function () {
		...
		var params = { id: $('.txtNumeroFiscalizacao', Processo.settings.container).val(), processoId: $('.hdnProcessoId', Processo.settings.container).val() };

		Modal.abrir(Processo.settings.urls.associarFiscalizacao, null, function (container) {
			FiscalizacaoListar.load(container, { associarFuncao: Processo.callBackAssociarFiscalizacao });
			Modal.defaultButtons(container);
		});
	},
	...
	callBackAssociarFiscalizacao: function (Fiscalizacao) {
		if ($('.txtNumeroFiscalizacao', Processo.settings.container).val() == Fiscalizacao.Id) {
			return true;
		}

		var params = { fiscalizacaoId: Fiscalizacao.Id };
		var retorno = Processo.obterAjax(Processo.settings.urls.obterFiscalizacao, params, $('.divFiscalizacao', Processo.settings.containerModal));

		if (!retorno.EhValido) {
			return retorno.Msg;
		}

		$('.txtNumeroFiscalizacao', Processo.settings.container).val(Fiscalizacao.Id);
		$('.hdnFiscalizacaoSituacao', Processo.settings.container).val(Fiscalizacao.SituacaoId);

		Processo.configurarAssociarMultiplo();

		$('.btnLimparFiscalizacao', Processo.settings.container).removeClass('hide');
		$('.btnBuscarFiscalizacao', Processo.settings.container).addClass('hide');
		$('.spnVisualizarFiscalizacao', Processo.settings.container).removeClass('hide');

		Processo.callBackEditarInteressado(retorno.Autuado);
	...
	onLimparFiscalizacao: function () {
		...
		var fiscalizacao = $('.txtNumeroFiscalizacao', Processo.settings.container).val() || 0;

		if (!Processo.validarDesassociarFicalizacao(fiscalizacao)) {
			return;
		}

		$('.txtNumeroFiscalizacao', Processo.settings.container).val('');
		$('.btnLimparFiscalizacao', Processo.settings.container).addClass('hide');
		$('.btnLimparFiscalizacao', Processo.settings.container).addClass('hide');
		$('.btnBuscarFiscalizacao', Processo.settings.container).removeClass('hide');
		$('.spnVisualizarFiscalizacao', Processo.settings.container).addClass('hide');

		$('.divFiscalizacao', Processo.settings.container).empty();
	...
	validarDesassociarFicalizacao: function (fiscalizacaoId) {
		...
		if (fiscalizacaoId > 0) {
			$.ajax({ url: Processo.settings.urls.desassociarFiscalizacaoValidar, data: JSON.stringify({ fiscalizacaoId: fiscalizacaoId }), type: 'POST', typeData: 'json',
				contentType: 'application/json; charset=utf-8', cache: false, async: false,
				error: function (XMLHttpRequest, textStatus, erroThrown) {
					Aux.error(XMLHttpRequest, textStatus, erroThrown, MasterPage.getContent(Processo.settings.container));
				},
	...
	onAbrirVisualizarFiscalizacao: function () {
		var id = parseInt($('.txtNumeroFiscalizacao', Processo.settings.container).val());
		var params = { id: id };

		Modal.abrir(Processo.settings.urls.visualizarFiscalizacao, params, function (container) {
			Modal.defaultButtons(container);
		}, Modal.tamanhoModalGrande);
	},
	...
	montarObjetoProcesso: function () {
		...
		if ($('.divPossuiSEP', Processo.settings.container).hasClass('hide')) {
			objetoProcesso.PossuiSEP = ($('.hdnPossuiSEP', Processo.settings.container).val().toLowerCase() == 'true');
		}
		else {
			objetoProcesso.PossuiSEP = $('.rdbProcessoPossuiSEP:checked', Processo.settings.container).val() == 1 ? true : $('.rdbProcessoPossuiSEP:checked', Processo.settings.container).val() == 2 ? false : undefined;
		}

		if (objetoProcesso.PossuiSEP) {
			objetoProcesso.NumeroAutuacao = $('.txtNumeroAutuacao', Processo.settings.container).val() == 'Gerado pelo sistema SEP' ? '' : $('.txtNumeroAutuacao', Processo.settings.container).val();
			objetoProcesso.DataAutuacao = { DataTexto: $('.txtDataAutuacao', Processo.settings.container).val() };
		}
		...
		objetoProcesso.Fiscalizacao.Id = $('.txtNumeroFiscalizacao', Processo.settings.container).val();
		objetoProcesso.Fiscalizacao.SituacaoId = $('.hdnFiscalizacaoSituacao', Processo.settings.container).val();
	...
	autuar: function () {
		MasterPage.carregando(true);
		var params = { processo: Processo.montarObjetoProcesso() };
