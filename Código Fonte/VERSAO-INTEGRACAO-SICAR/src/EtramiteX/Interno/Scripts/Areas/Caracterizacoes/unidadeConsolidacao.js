...
UnidadeConsolidacao = {
	...
	adicionarResponsavelTecnico: function () {
		...
		$(".rbARTCargoFuncaoSim", container).trigger('click');
	...
	obter: function () {
		...
		$('.gridCultivares tbody tr:not(.trTemplate) .hdnItemJson', container).each(function () {
			objeto.Cultivares.push(JSON.parse($(this).val()));
		});

		$('.gridResponsaveis tbody tr:not(.trTemplate) .hdnItemJson', container).each(function () {
