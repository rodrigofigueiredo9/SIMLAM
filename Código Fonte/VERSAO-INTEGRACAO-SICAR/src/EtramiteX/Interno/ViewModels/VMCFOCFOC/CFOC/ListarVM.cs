...
namespace Tecnomapas.EtramiteX.Interno.ViewModels.VMCFOCFOC.CFOC
{
	public class ListarVM
	{
		...
		public ListarVM(List<QuantPaginacao> quantPaginacao, List<Lista> situacoes)
		{
			...
			SituacaoLista = ViewModelHelper.CriarSelectList(situacoes.Where(x=> Convert.ToInt32(x.Id) != (int)eDocumentoFitossanitarioSituacao.EmElaboracao).ToList(), true, true);
