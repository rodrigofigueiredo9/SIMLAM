...
<script type="text/javascript">	
	...
	TermoCPFARLCR.settings.urls.validarAdicionarARL = '<%= Url.Action("ValidarAdicionarARL", "Termo", new { area = "Especificidades" }) %>';

	TermoCPFARLCR.settings.Mensagens = <%= Model.Mensagens %>;
	TermoCPFARLCR.settings.dominialidadeID = <%= Model.Especificidade.DominialidadeID %>;
</script>
	...
	<fieldset class="block boxBranca">
		...
		<div class="block">
			<div class="coluna20 append1">
				...
				<%= Html.TextBox("NumeroAverbacao", Model.Especificidade.NumeroAverbacao, ViewModelHelper.SetaDisabled(Model.IsVisualizar, new { @class = "text txtNumeroAverbacao", @maxlength = "20" }))%>
			</div>

			<div class="coluna20">
				<label for="DataEmissao">Data da emissão *</label>
				<%= Html.TextBox("DataEmissao", Model.Especificidade.DataEmissao.DataTexto, ViewModelHelper.SetaDisabled(Model.IsVisualizar, new { @class = "text maskData txtDataEmissao" }))%>
			</div>
		</div>
	</fieldset>
</fieldset>
