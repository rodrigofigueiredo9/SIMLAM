...
{
	public class LaudoFundiarioSimplificadoDa
	{
		#region Propriedades
		...
		public LaudoFundiarioSimplificadoDa(string strBancoDeDados = null)
		{
		...
		internal void Salvar(LaudoFundiarioSimplificado laudo, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				Comando comando = bancoDeDados.CriarComando(@"select e.id from {0}esp_laudo_fundiario_simplific e where e.titulo = :titulo", EsquemaBanco);
				comando.AdicionarParametroEntrada("titulo", laudo.Titulo.Id, DbType.Int32);
				id = bancoDeDados.ExecutarScalar(comando);

				if (id != null && !Convert.IsDBNull(id))
				{
					comando = bancoDeDados.CriarComando(@"update {0}esp_laudo_fundiario_simplific e set e.titulo = :titulo, e.protocolo = :protocolo, 
					e.destinatario = :destinatario, e.data_vistoria = :data_vistoria, e.objetivo = :objetivo, e.descricao_parecer
                    = :descricao_parecer, e.tid = :tid where e.titulo = :titulo", EsquemaBanco);
				...
				{
					comando = bancoDeDados.CriarComando(@"insert into {0}esp_laudo_fundiario_simplific(id, titulo, protocolo, destinatario,  
					data_vistoria, objetivo, descricao_parecer, tid) values ({0}seq_esp_laudo_visto_licenciam.nextval, :titulo, :protocolo, :destinatario, 
					 :data_vistoria, :objetivo, :descricao_parecer, :tid) returning id into :id", EsquemaBanco);
				...
				if (id == null || Convert.IsDBNull(id))
				{
					laudo = laudo ?? new LaudoFundiarioSimplificado();
				...
				Historico.Gerar(Convert.ToInt32(laudo.Titulo.Id), eHistoricoArtefatoEspecificidade.laudofundiariosimplificado, acao, bancoDeDados);
		...
		internal void Excluir(int titulo, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				Comando comando = bancoDeDados.CriarComando(@"update {0}esp_laudo_fundiario_simplific c set c.tid = :tid where c.titulo = :titulo", EsquemaBanco);
				...
				Historico.Gerar(titulo, eHistoricoArtefatoEspecificidade.laudofundiariosimplificado, eHistoricoAcao.excluir, bancoDeDados);

				#endregion

				#region Apaga os dados da especificidade

				comando = bancoDeDados.CriarComando(@"delete from {0}esp_laudo_fundiario_simplific e where e.titulo = :titulo", EsquemaBanco);
		...
		internal LaudoFundiarioSimplificado Obter(int titulo, BancoDeDados banco = null)
		{
			LaudoFundiarioSimplificado especificidade = new LaudoFundiarioSimplificado();
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				select e.id, e.tid, e.protocolo, e.descricao_parecer, e.data_vistoria, e.objetivo, n.numero, n.ano, p.requerimento, p.protocolo protocolo_tipo, e.destinatario, 
				(select distinct nvl(pe.nome, pe.razao_social) from {0}hst_esp_laudo_fundia_simplific he, {0}hst_pessoa pe where he.destinatario_id = pe.pessoa_id and he.destinatario_tid = pe.tid
				...
				from {0}esp_laudo_fundiario_simplific e, {0}tab_titulo_numero n, {0}tab_protocolo p where n.titulo(+) = e.titulo and e.protocolo = p.id(+) and e.titulo = :titulo", EsquemaBanco);
		...
		internal Laudo ObterDadosPDF(int titulo, BancoDeDados banco = null)
		{
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				Comando comando = bancoDeDados.CriarComando(@"select e.destinatario, e.data_vistoria, e.objetivo, e.descricao_parecer from {0}esp_laudo_fundiario_simplific e where e.titulo = :id", EsquemaBanco);
