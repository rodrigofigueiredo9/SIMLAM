...
{
	public class LaudoVistoriaLicenciamentoDa
	{
		#region Propriedades
		...
		public LaudoVistoriaLicenciamentoDa(string strBancoDeDados = null)
		{
		...
		internal void Salvar(LaudoVistoriaLicenciamento laudo, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				Comando comando = bancoDeDados.CriarComando(@"select e.id from {0}esp_laudo_vistoria_licenciame e where e.titulo = :titulo", EsquemaBanco);
				comando.AdicionarParametroEntrada("titulo", laudo.Titulo.Id, DbType.Int32);
				id = bancoDeDados.ExecutarScalar(comando);

				if (id != null && !Convert.IsDBNull(id))
				{
					comando = bancoDeDados.CriarComando(@"update {0}esp_laudo_vistoria_licenciame e set e.titulo = :titulo, e.protocolo = :protocolo, 
					e.destinatario = :destinatario, e.responsavel = :responsavel, e.conclusao = :conclusao,
				...
				{
					comando = bancoDeDados.CriarComando(@"insert into {0}esp_laudo_vistoria_licenciame(id, titulo, protocolo, destinatario, responsavel, conclusao, 
					data_vistoria, objetivo, consideracao, restricao, descricao_parecer, tid) values ({0}seq_esp_laudo_visto_licenciam.nextval, :titulo, :protocolo, :destinatario, 
					:responsavel, :conclusao, :data_vistoria, :objetivo, :consideracao, :restricao, :descricao_parecer, :tid) returning id into :id", EsquemaBanco);
				...
				if (id == null || Convert.IsDBNull(id))
				{
					laudo = laudo ?? new LaudoVistoriaLicenciamento();
				...
				Historico.Gerar(Convert.ToInt32(laudo.Titulo.Id), eHistoricoArtefatoEspecificidade.laudovistorialicenciamento, acao, bancoDeDados);
		...
		internal void Excluir(int titulo, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				Comando comando = bancoDeDados.CriarComando(@"update {0}esp_laudo_vistoria_licenciame c set c.tid = :tid where c.titulo = :titulo", EsquemaBanco);
				...
				Historico.Gerar(titulo, eHistoricoArtefatoEspecificidade.laudovistorialicenciamento, eHistoricoAcao.excluir, bancoDeDados);

				#endregion

				#region Apaga os dados da especificidade

				comando = bancoDeDados.CriarComando(@"delete from {0}esp_laudo_vistoria_licenciame e where e.titulo = :titulo", EsquemaBanco);
		...
		internal LaudoVistoriaLicenciamento Obter(int titulo, BancoDeDados banco = null)
		{
			LaudoVistoriaLicenciamento especificidade = new LaudoVistoriaLicenciamento();

			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				#region Dados da Especificidade

				Comando comando = bancoDeDados.CriarComando(@"select e.id, e.tid, e.protocolo, e.responsavel, e.conclusao, e.data_vistoria,
				...
				(select distinct nvl(pe.nome, pe.razao_social) from {0}hst_esp_laudo_visto_licenciam he, {0}hst_pessoa pe where he.destinatario_id = pe.pessoa_id and he.destinatario_tid = pe.tid
		...
		internal Laudo ObterDadosPDF(int titulo, BancoDeDados banco = null)
		{
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				Comando comando = bancoDeDados.CriarComando(@"select e.destinatario, e.protocolo, e.responsavel, lp.texto conclusao, e.data_vistoria, e.objetivo, e.consideracao, 
				e.restricao, e.descricao_parecer from {0}esp_laudo_vistoria_licenciame e, {0}lov_esp_conclusao lp where e.conclusao = lp.id and e.titulo = :id", EsquemaBanco);
