...
{
	public class AutorizacaoExploracaoFlorestalDa
	{
		#region Propriedades
		...
		public AutorizacaoExploracaoFlorestalDa(string strBancoDeDados = null)
		{
		...
		internal void Excluir(int titulo, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				Comando comando = bancoDeDados.CriarComando(@"update {0}esp_auto_exploracao_florestal c set c.tid = :tid where c.titulo = :titulo", EsquemaBanco);
				...
				Historico.Gerar(titulo, eHistoricoArtefatoEspecificidade.autorizacaoexploracaoflorestal, eHistoricoAcao.excluir, bancoDeDados);

				#endregion

				#region Apaga os dados da especificidade

				comando = bancoDeDados.CriarComando(@"delete from {0}esp_auto_exploracao_florestal e where e.titulo = :titulo", EsquemaBanco);
		...
		internal AutorizacaoExploracaoFlorestal Obter(int titulo, BancoDeDados banco = null)
		{
			AutorizacaoExploracaoFlorestal especificidade = new AutorizacaoExploracaoFlorestal();

			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				#region Dados da Especificidade

				Comando comando = bancoDeDados.CriarComando(@"select e.id, e.tid, e.protocolo, e.observacao, n.numero, n.ano, p.requerimento, p.protocolo protocolo_tipo, e.destinatario, 
				(select distinct nvl(pe.nome, pe.razao_social) from {0}hst_esp_auto_explora_florestal he, {0}hst_pessoa pe where he.destinatario_id = pe.pessoa_id and he.destinatario_tid = pe.tid
				...
				from {0}esp_auto_exploracao_florestal e, {0}tab_titulo_numero n, {0}tab_protocolo p where n.titulo(+) = e.titulo and e.protocolo = p.id(+) and e.titulo = :titulo", EsquemaBanco);
				...
				using (IDataReader reader = bancoDeDados.ExecutarReader(comando))
				{
					if (reader.Read())
					{
						...
						especificidade.Observacao = reader["observacao"].ToString();
