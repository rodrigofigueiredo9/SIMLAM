...
{
	public class LicencaAmbientalRegularizacaoDa
	{
		#region Propriedades
		...
		public LicencaAmbientalRegularizacaoDa(string strBancoDeDados = null)
		{
		...
		internal void Salvar(LicencaAmbientalRegularizacao Licenca, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				{
					comando = bancoDeDados.CriarComando(@"insert into {0}esp_licenca_ambiental_regulari(id, titulo, protocolo, destinatario, tid, barragem_id, caracterizacao_id) values ({0}seq_esp_licenca_ambiental_regu.nextval, :titulo, 
				...
				if (id == null || Convert.IsDBNull(id))
				{
					Licenca = Licenca ?? new LicencaAmbientalRegularizacao();
				...
				Historico.Gerar(Convert.ToInt32(Licenca.Titulo.Id), eHistoricoArtefatoEspecificidade.licencaambientalregularizacao, acao, bancoDeDados);
		...
		internal void Excluir(int titulo, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				Comando comando = bancoDeDados.CriarComando(@"update {0}esp_licenca_ambiental_regulari c set c.tid = :tid where c.titulo = :titulo", EsquemaBanco);
				...
				Historico.Gerar(titulo, eHistoricoArtefatoEspecificidade.licencaambientalregularizacao, eHistoricoAcao.excluir, bancoDeDados);

				#endregion

				#region Apaga os dados da especificidade

				comando = bancoDeDados.CriarComando(@"delete from {0}esp_licenca_ambiental_regulari e where e.titulo = :titulo", EsquemaBanco);
		...
		internal LicencaAmbientalRegularizacao Obter(int titulo, BancoDeDados banco = null)
		{
			LicencaAmbientalRegularizacao especificidade = new LicencaAmbientalRegularizacao();
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				(select distinct nvl(pe.nome, pe.razao_social) from {0}hst_esp_licenca_ambiental_regu he, {0}hst_pessoa pe where he.destinatario_id = pe.pessoa_id and he.destinatario_tid = pe.tid
				...
				(select t.caracterizacao_tid from {0}hst_esp_licenca_ambiental_regu t where t.id = (select max(id) from {0}hst_esp_licenca_ambiental_regu where especificidade_id = e.id)) caracterizacao_tid 
				from {0}esp_licenca_ambiental_regulari e, {0}tab_titulo_numero n, {0}tab_protocolo p where n.titulo(+) = e.titulo and e.protocolo = p.id(+) and e.titulo = :titulo", EsquemaBanco);
