...
{
	public class LicencaOperacaoDa
	{
		#region Propriedades
		...
		public LicencaOperacaoDa(string strBancoDeDados = null)
		{
		...
		internal void Salvar(LicencaOperacao Licenca, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				Comando comando = bancoDeDados.CriarComando(@"select e.id from {0}esp_licenca_operacao e where e.titulo = :titulo", EsquemaBanco);
				comando.AdicionarParametroEntrada("titulo", Licenca.Titulo.Id, DbType.Int32);
				id = bancoDeDados.ExecutarScalar(comando);

				if (id != null && !Convert.IsDBNull(id))
				{
					comando = bancoDeDados.CriarComando(@"update {0}esp_licenca_operacao e set e.titulo = :titulo, e.protocolo = :protocolo, e.destinatario = :destinatario, e.barragem_id = :barragem_id, e.caracterizacao_id = (select distinct t.barragem from {0}crt_barragem_barragens t where t.id = :barragem_id), e.tid = :tid where e.titulo = 
                        :titulo", EsquemaBanco);
				...
				{
					comando = bancoDeDados.CriarComando(@"insert into {0}esp_licenca_operacao(id, titulo, protocolo, destinatario, tid, barragem_id, caracterizacao_id) values ({0}seq_esp_licenca_operacao.nextval, :titulo, 
				...
				if (id == null || Convert.IsDBNull(id))
				{
					Licenca = Licenca ?? new LicencaOperacao();
				...
				Historico.Gerar(Convert.ToInt32(Licenca.Titulo.Id), eHistoricoArtefatoEspecificidade.licencaoperacao, acao, bancoDeDados);
		...
		internal LicencaOperacao Obter(int titulo, BancoDeDados banco = null)
		{
			LicencaOperacao especificidade = new LicencaOperacao();
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				(select distinct nvl(pe.nome, pe.razao_social) from {0}hst_esp_licenca_operacao he, {0}hst_pessoa pe where he.destinatario_id = pe.pessoa_id and he.destinatario_tid = pe.tid
				...
				(select t.caracterizacao_tid from {0}hst_esp_licenca_operacao t where t.id = (select max(id) from {0}hst_esp_licenca_operacao where especificidade_id = e.id)) caracterizacao_tid 
				from {0}esp_licenca_operacao e, {0}tab_titulo_numero n, {0}tab_protocolo p where n.titulo(+) = e.titulo and e.protocolo = p.id(+) and e.titulo = :titulo", EsquemaBanco);
		...
		internal Licenca ObterDadosPDF(int titulo, BancoDeDados banco = null)
		{
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				Comando comando = bancoDeDados.CriarComando(@"select e.destinatario from {0}esp_licenca_operacao e where titulo = :id", EsquemaBanco);
