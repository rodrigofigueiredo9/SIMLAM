...
namespace Tecnomapas.EtramiteX.Interno.Model.Extensoes.Especificidades.ModuloTermo.Data
{
	public class TermoCPFARLCDa
	{
		...
		public TermoCPFARLCDa(string strBancoDeDados = null)
		{
		...
		internal void Excluir(int titulo, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				Comando comando = bancoDeDados.CriarComando(@"update {0}esp_termo_cpfarlc c set c.tid = :tid where c.titulo = :titulo", EsquemaBanco);
				...
				begin 
					delete from {0}esp_tcpfarlc_rec_resp e where e.especificidade = (select t.id from {0}esp_termo_cpfarlc t where t.titulo = :titulo);
					delete from {0}esp_tcpfarlc_ced_resp e where e.especificidade = (select t.id from {0}esp_termo_cpfarlc t where t.titulo = :titulo);
					delete from {0}esp_tcpfarlc_ced_rese e where e.especificidade = (select t.id from {0}esp_termo_cpfarlc t where t.titulo = :titulo);
					delete from {0}esp_termo_cpfarlc e where e.titulo = :titulo;
				end;", EsquemaBanco);
		...
		internal Titulo ObterSituacaoTituloPorAPMP(int matriculaPosseCedente, int matriculaPosseReceptor, int tituloID, BancoDeDados banco = null)
		{
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				Comando comando = bancoDeDados.CriarComando(@"select t.situacao_id, t.situacao_texto, t.modelo_sigla, t.numero from {0}esp_termo_cpfarlc e,
					{0}lst_titulo t where t.titulo_id = e.titulo and e.dominio_cedente = :dominio_cedente and e.dominio_receptor = :dominio_receptor 
					and e.titulo <> :titulo", EsquemaBanco);

				comando.AdicionarParametroEntrada("dominio_cedente", matriculaPosseCedente, DbType.Int32);
				comando.AdicionarParametroEntrada("dominio_receptor", matriculaPosseReceptor, DbType.Int32);
				...
				using (IDataReader reader = bancoDeDados.ExecutarReader(comando))
				{
					if (reader.Read())
					{
						...
						titulo.Modelo.Nome = reader.GetValue<string>("modelo_sigla");
						titulo.Numero.Texto = reader.GetValue<string>("numero");
					}
				}
			}
		...
		internal bool ValidarApmpAssociadaEmpreendimento(int apmpID, int empreendimentoId, BancoDeDados banco = null)
		{
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				Comando comando = bancoDeDados.CriarComando(@"select count(d.id) total from{0} crt_dominialidade_dominio d, {0}crt_dominialidade c where d.dominialidade = c.id 
					and d.id = :apmp and c.empreendimento = :empreendimento", EsquemaBanco);

				comando.AdicionarParametroEntrada("apmp", apmpID, DbType.Int32);
				...
				using (IDataReader reader = bancoDeDados.ExecutarReader(comando))
				{
					if (reader.Read())
					{
						if (reader.GetValue<int>("total") == 0)
						{
							retono = true;
						}
					}
		...
		internal string ObterTipoAPMP(int apmpID, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				Comando comando = bancoDeDados.CriarComando(@"select l.texto situacao from {0}crt_dominialidade_dominio d, {0}lov_crt_domin_dominio_tipo l 
					where l.id = d.tipo  and d.id = :apmp", EsquemaBanco);
		...
		internal List<TituloModeloLst> ObterTitulosCedenteReceptor(int dominioCedente, int dominioReceptor, BancoDeDados banco = null)
		{
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				from esp_termo_cpfarlc e, tab_titulo t, lov_titulo_situacao ls, tab_titulo_modelo tm, tab_titulo_numero tn  
