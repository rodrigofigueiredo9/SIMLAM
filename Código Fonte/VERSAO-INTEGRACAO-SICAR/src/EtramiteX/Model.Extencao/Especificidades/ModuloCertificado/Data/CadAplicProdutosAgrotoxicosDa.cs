...
{
	public class CadAplicProdutosAgrotoxicosDa
	{
		#region Propriedades
		...
		public CadAplicProdutosAgrotoxicosDa(string strBancoDeDados = null)
		{
		...
		internal CadAplicProdutosAgrotoxicos Obter(int titulo, BancoDeDados banco = null)
		{
			CadAplicProdutosAgrotoxicos especificidade = new CadAplicProdutosAgrotoxicos();
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				(select distinct nvl(pe.nome, pe.razao_social) from {0}hst_esp_cad_aplic_prod_agr he, {0}hst_pessoa pe where he.destinatario_id = pe.pessoa_id and he.destinatario_tid = pe.tid
				...
				from {0}esp_cad_aplic_prod_agr e, {0}tab_titulo_numero n, {0}tab_protocolo p where n.titulo(+) = e.titulo and e.protocolo = p.id(+) and e.titulo = :titulo", EsquemaBanco);
