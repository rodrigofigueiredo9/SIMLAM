...
{
	public class EscrituraPublicaCompraVendaDa
	{
		#region Propriedades
		...
		public EscrituraPublicaCompraVendaDa(string strBancoDeDados = null)
		{
		...
		internal void Excluir(int titulo, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				//
				Comando comando = bancoDeDados.CriarComando(@"update {0}esp_escritura_pub_comp_ven c set c.tid = :tid where c.titulo = :titulo", EsquemaBanco);
				...
				Historico.Gerar(titulo, eHistoricoArtefatoEspecificidade.escriturapublicacompravenda, eHistoricoAcao.excluir, bancoDeDados);

				#endregion

				#region Apaga os dados da especificidade

				comando = bancoDeDados.CriarComando(@"delete from {0}esp_escritura_pub_comp_ven e where e.titulo = :titulo", EsquemaBanco);
		...
		internal EscrituraPublicaCompraVenda Obter(int titulo, BancoDeDados banco = null)
		{
			EscrituraPublicaCompraVenda especificidade = new EscrituraPublicaCompraVenda();
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				(select distinct nvl(pe.nome, pe.razao_social) from {0}hst_esp_escritura_pub_comp_ven he, {0}hst_pessoa pe where he.destinatario_id = pe.pessoa_id and he.destinatario_tid = pe.tid
				...
				from {0}esp_escritura_pub_comp_ven e, {0}tab_titulo_numero n, {0}tab_protocolo p where n.titulo(+) = e.titulo and e.protocolo = p.id(+) and e.titulo = :titulo", EsquemaBanco);
