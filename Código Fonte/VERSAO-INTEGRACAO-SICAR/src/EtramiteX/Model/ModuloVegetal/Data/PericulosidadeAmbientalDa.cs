...
{
	public class PericulosidadeAmbientalDa
	{
		#region Propriedades
		...
		internal void Salvar(ConfiguracaoVegetalItem periculosidadeAmbiental, BancoDeDados banco = null)
		{
			if (periculosidadeAmbiental == null)
			{
				throw new Exception("periculosidadeAmbiental é nulo.");
			}

			if (periculosidadeAmbiental.Id <= 0)
			{
				Criar(periculosidadeAmbiental, banco);
			}
			else
			{
				Editar(periculosidadeAmbiental, banco);
			}
		}
		...
		internal List<ConfiguracaoVegetalItem> Listar()
		{
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia())
			{
				#region Periculosidades Ambientais

				Comando comando = bancoDeDados.CriarComando(@"select id, texto, tid from tab_peric_ambiental order by texto", EsquemaBanco);
