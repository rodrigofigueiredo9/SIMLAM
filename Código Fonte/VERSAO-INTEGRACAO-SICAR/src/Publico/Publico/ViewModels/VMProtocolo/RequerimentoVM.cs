...
namespace Tecnomapas.EtramiteX.Publico.ViewModels.VMProtocolo
{
	public class RequerimentoVM
	{
		...
		private bool _isRequerimentoProcesso = true;
		public bool IsRequerimentoProcesso
		{
			get { return _isRequerimentoProcesso; }
			set { _isRequerimentoProcesso = value; }
		}
		...
		private void CarregarAtividadeSolicitada()
		{
			foreach (var item in this.AtividadesSolicitadas)
			{
				...
				atividade.IsRequerimento = !this.IsRequerimentoProcesso;
				atividade.RetirarIconeExcluirAtividade = true;
				atividade.SetorId = item.SetorId;				
				atividade.Protocolo = item.Protocolo;
		...
		public void ResetIdRelacionamento()
		{
			...
			foreach (AtividadeSolicitadaVM atividade in this.AtividadesSolicitadasVM)
			{
				...
				{
					item.IdRelacionamento = 0;
				}
			}

			foreach (Atividade atividade in this.AtividadesSolicitadas)
			{
				atividade.IdRelacionamento = 0;
