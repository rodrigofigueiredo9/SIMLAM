...
namespace Tecnomapas.EtramiteX.WindowsService.ProcessOperacoesPublicoGeo.Business
{
	public class ProjetoBus
	{
		...
		public void SalvarArquivo(Arquivo arquivo, int projeto, int tipoArquivo)
		{
			...
			_dataAccess.SetArquivoDisponivel(arquivo.Id ?? 0, projeto, tipoArquivo);
			_banco.Commit();
		}
		...
		public void SetConcluidoNaFila(int projeto, int tipo)
		{
			_banco.IniciarTransacao();
			_dataAccess.SetConcluidoNaFila(projeto, tipo);
			_banco.Commit();
		}

		public void SetProcessado(int projeto)
		{
			_banco.IniciarTransacao();
			_dataAccess.SetProcessado(projeto);
			_banco.Commit();
		}
		...
		public void ApagarGeometriasTemporariasTrackmaker(int projeto)
		{
			_banco.IniciarTransacao();
			_dataAccess.ApagarGeometriasTemporariasTrackmaker(projeto);
			_banco.Commit();
		}
		...
		public List<Hashtable> ValidarErrosEspaciais(int projeto)
		{
			return _dataAccess.ValidarErrosEspaciais(projeto);
		}

		public List<Hashtable> ValidarObrigatoriedades(int projeto)
		{
			return _dataAccess.ValidarObrigatoriedades(projeto);
		}

		public List<Hashtable> ValidarAtributos(int projeto)
		{
			return _dataAccess.ValidarAtributos(projeto);
		}

		public List<Hashtable> ContabilizarGeometrias(int projeto)
		{
			return _dataAccess.ContabilizarGeometrias(projeto);
		}
	}
}