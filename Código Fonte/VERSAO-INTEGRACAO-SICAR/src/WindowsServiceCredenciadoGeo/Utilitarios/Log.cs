	...
	public class Log
	{
		...
		public static void GerarLogBanco(Exception exc)
		{
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia())
			{
				...
				comando.AdicionarParametroEntrada("source", DbType.String, 1000, nomeExe);
				comando.AdicionarParametroEntrada("machine", DbType.String, 1000, Environment.MachineName);
				comando.AdicionarParametroEntrada("mensagem", DbType.String, 4000, strMsg);
			...
			catch (Exception logExc)
			{
				...
				try
				{
					string excMsg = String.Format("Exception serviço: {0} \r\n Log:{1}",
						GerarTextoExc(exc), GerarTextoExc(logExc));
					GerarLog(excMsg);
				}
				catch
				{
		...
		public static void GerarLog(string Mensagem)
		{
			...
			try
			{
				...
				if (!string.IsNullOrWhiteSpace(ConfigurationManager.AppSettings["ArquivoLog"]))
				{
					nomeExe = ConfigurationManager.AppSettings["ArquivoLog"];
				}
