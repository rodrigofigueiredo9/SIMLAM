	...
	public class InformacaoCorteDa
	{
		...
		private int Criar(InformacaoCorte caracterizacao, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco, EsquemaCredenciadoBanco))
			{
				...
				(id, tid, empreendimento, data_informacao, area_flor_plantada, interno_id, interno_tid, codigo) values
				(seq_informacao_corte.nextval, :tid, :empreendimento_id, :data_informacao, :area_flor_plantada, :interno_id, :interno_tid, :codigo)
				...
				foreach (var item in caracterizacao.InformacaoCorteTipo)
				{
					...
					foreach (var destinacao in item.InformacaoCorteDestinacao)
					{
						...
						(select s.id from {0}lov_inf_codigo_sefaz s where s.inf_corte_produto = :produto and s.inf_destinacao_material = :dest_material and rownum = 1))", EsquemaCredenciadoBanco);
		...
		private void Editar(InformacaoCorte caracterizacao, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco, EsquemaCredenciadoBanco))
			{
				...
				comando = bancoDeDados.CriarComando(@"delete from {0}crt_inf_corte_dest_material c where exists
					(select 1 from {0}crt_inf_corte_tipo t where t.corte_id = :corte_id and t.id = c.tipo_corte_id)", EsquemaCredenciadoBanco);
		...
		internal InformacaoCorte Obter(int id, BancoDeDados banco = null, bool simplificado = false)
		{
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco, EsquemaCredenciadoBanco))
			{
				...
				select c.id, c.tid, c.codigo, c.interno_id, c.interno_tid, c.empreendimento, c.data_informacao, c.area_flor_plantada
		...
		internal List<Lista> ObterProdutos(int destinacaoId, BancoDeDados banco = null)
		{
			List<Lista> retorno = new List<Lista>();
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco, EsquemaCredenciadoBanco))
			{
				...
				inner join {0}lov_crt_produto_inf_corte p
				on (p.id = s.inf_corte_produto)
				where inf_destinacao_material = :inf_destinacao_material", EsquemaCredenciadoBanco);
		...
		internal bool PossuiCaracterizacaoEmAberto(int empreendimentoId, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco, EsquemaCredenciadoBanco))
			{
		...
		internal bool CaracterizacaoEmAberto(int id, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco, EsquemaCredenciadoBanco))
			{
		...
		private int ObterProximoCodigo(int empreendimento, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				BEGIN
					UPDATE idaf.crt_inf_corte_codigo
						SET codigo = codigo + 1
						WHERE empreendimento = (select e.interno from {1}tab_empreendimento e where e.id = :empreendimento and rownum = 1)
