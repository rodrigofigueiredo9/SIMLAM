...
namespace Tecnomapas.EtramiteX.Credenciado.Model.ModuloPessoa.Data
{
	public class PessoaInternoDa
	{
		...
		public PessoaInternoDa(string strBancoDeDados = null)
		{
		...
		internal Resultados<ProfissaoLst> FiltrarProfissao(Filtro<ProfissaoFiltro> filtros, BancoDeDados banco = null)
		{
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				comandtxt += comando.FiltroAndLike("p.texto", "profissao_texto", filtros.Dados.Profissao, true);
				...
				comando.DbCommand.CommandText = "select count(*) from (select p.id, p.texto, 1 similaridade from tab_profissao p where p.id > 0 " + comandtxt + ")";
