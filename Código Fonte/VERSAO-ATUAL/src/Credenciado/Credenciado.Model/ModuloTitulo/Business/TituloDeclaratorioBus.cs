...
namespace Tecnomapas.EtramiteX.Credenciado.Model.ModuloTitulo.Business
{
	public class TituloDeclaratorioBus
	{
		...
		public void AcerrarTitulo(int requerimentoID, BancoDeDados banco = null)
		{
			List<Titulo> titulos = _da.ObterPorRequerimento(requerimentoID, eTituloSituacao.Valido);

			foreach (var item in titulos)
			{
				Titulo tituloAtual = _da.Obter(item.Id);
				tituloAtual.Modelo = _busModelo.Obter(tituloAtual.Modelo.Id);
				tituloAtual.Situacao.Id = (int)eTituloSituacao.EncerradoDeclaratorio;
				tituloAtual.MotivoEncerramentoId = (int)eTituloMotivoEncerramento.EncerradoPorCancelamentoProjetoDigital;

				AlterarSituacao(tituloAtual, validar: false);
			}
		}
