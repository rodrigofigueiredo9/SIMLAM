	...
	public class EmissaoCFOCDa
	{
		...
		internal EmissaoCFOC Obter(int id, bool simplificado = false, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco, EsquemaCredenciado))
			{
				...
				Comando comando = bancoDeDados.CriarComando(@"select c.tid, c.tipo_numero, c.numero, c.data_emissao, c.situacao, c.empreendimento, c.possui_laudo_laboratorial, 
				...
				from (select cp.id, cp.tid, cp.lote, l.codigo_uc || l.ano || lpad(l.numero, 4, '0') codigo_lote, l.data_criacao, c.id cultura_id, c.texto cultura, cc.id cultivar_id, cc.cultivar,
                case when li.exibe_kilos is null then li.quantidade else cp.quantidade end as quantidade, 
