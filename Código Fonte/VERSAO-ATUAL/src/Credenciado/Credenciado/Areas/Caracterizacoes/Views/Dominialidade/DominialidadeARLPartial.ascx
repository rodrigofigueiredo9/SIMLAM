	...
	<div class="block filtroCorpo">
		...
		<div class="block">
			...
			<div class="coluna25">
				...
				<%= Html.TextBox("Dominialidade.AreaARLNaoCaracterizada", Model.Caracterizacao.ARLNaoCaracterizada.ToStringTrunc(), new { @class = "text disabled txtARLNaoCaracterizada", @disabled = "disabled" })%>
			</div>
		</div>
		<%bool mostrar = Model.Caracterizacao.PossuiPorcentagemMinimaARL &&
						Model.Caracterizacao.Dominios.SelectMany(dominio => dominio.ReservasLegais)
						.Count(r => r.CompensacaoTipo == eCompensacaoTipo.Receptora || r.CompensacaoTipo == eCompensacaoTipo.Nulo) <= 0;%>
