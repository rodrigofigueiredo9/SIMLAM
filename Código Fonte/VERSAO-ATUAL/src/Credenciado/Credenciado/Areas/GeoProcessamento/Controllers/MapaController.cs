...
using Tecnomapas.EtramiteX.Credenciado.Areas.GeoProcessamento.ViewModels.VMMapa;
using Tecnomapas.EtramiteX.Credenciado.Model.ModuloGeoProcessamento.Business;
...
namespace Tecnomapas.EtramiteX.Credenciado.Controllers
{
	public class MapaController : DefaultController
	{
		...
		{
			MapaCoordenadaInternoBus bus = new MapaCoordenadaInternoBus();
		...
		{
			CoordenadaVM vm = new CoordenadaVM();
			vm.CoordenadaGeoUrl = Url.Content("~/Areas/Navegadores/Desenhador/DesenhadorCredenciado.swf");
			vm.Modo = modo;
			return PartialView(vm);
		}
