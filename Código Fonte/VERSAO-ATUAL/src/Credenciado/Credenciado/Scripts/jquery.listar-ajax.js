...
(function ($) {
		...
		init: function (options) {
			return this.each(function () {
				...
				container.delegate('.btnBuscar', 'click', function () {
					...
					filtrar(container);
					filtrosJsonString = JSON.stringify(jsonContents($('.filtroSerializarAjax', container)));
	...
	var jsonContents = function (ctr) {
		...
		'.maskCnpjParcial, .maskData, .maskNum3, maskNum4, .maskNum15, .maskNumEndereco, .maskAreaAbrangencia, ' +
	...
	var filtrar = function (container) {
		...
		serializedData = jsonContents($('.filtroSerializarAjax', container));

		if (settings.onBeforeFiltrar) {
			settings.onBeforeFiltrar(container, serializedData);
		}

		if (settings.customFiltrarFunction) {
			jsonReturned = settings.customFiltrarFunction(container, serializedData);
		} else {
			$.ajax({ url: $('.urlFiltrar', container).val(), data: JSON.stringify(serializedData), type: 'POST', typeData: 'json',
				...
				success: function (response, textStatus, XMLHttpRequest) {
					...
					if (settings.onAfterFiltrar) {
						settings.onAfterFiltrar(container, serializedData, jsonReturned);
					}
