...
$(function () {
	...
	$(document).ajaxComplete(MasterPage.validarAcesso);
...
MasterPage = {
	...
	redimensionarAcao: function () {
		...
		$('#central').css({ 'min-height': (maxBottom - top) + 'px' });
	},
				...
				$(".btnHistorico", context).click(function () {
					Modal.abrir(MasterPage.urlSobreItens, null, function (contextItens) {
						...
						$("p", contextItens).click(function () {
							...
							$.ajax({
								url: MasterPage.urlObterSobreItens,
	...
	tabIndex: function (options) {
		...
		if (!settings.ligado) {
			$(settings.container).find('a, area, button, input, object, select, textarea').each(function (idx, item) {
				...
				if ($(item).is('button, input[type="button"]')) {
					$(item).blur();
				}
			});
		} else {
...
Mascara = {
	...
	load: function (container) {
		...
		$(".maskMesAno").unbind(".mask").mask("99/9999");
		...
		$(".maskDecimal", container).unmask().maskMoney();
		$(".maskDecimal4", container).unmask().maskMoney({ precision: 4 });
...
Modal = {
	...
	confirma: function (options) {
		var defaults = {
			...
			forcarLoadCallback: false,     // Forçar executar a funcao de callBack do Load
		...
		if (settings.url) {
			Modal.abrir(settings.url, settings.urlData, _onModalLoad, settings.tamanhoModal, undefined, settings.forcarLoadCallback);
		} else {
	...
	defaultButtons: function (modal, okFunction, okLabel, okData, cancelFunction, cancelLabel, cancelData, removerFechar) {
		...
		buttonsContainer.find(".btnModalCancelar").removeClass('hide');
		...
		if (removerFechar) {
			...
			modal.keydown(function (e) {
				if (e.keyCode == 27) {
					Modal.fechar(modal.find('.modalBranco'));
				}
			});
		}
	},
	...
	abrir: function (url, data, onLoadCallbackName, tamanho, tituloTela, forcarLoadCallback) {
		...
		if (retorno == null || retorno == "") {
			if (forcarLoadCallback && typeof onLoadCallbackName != 'undefined') {
	...
	excluir: function (options) {
		...
		if (!settings.data) {
			settings.data = { id: settings.id };
			settings.urlConfirm = settings.urlConfirm + '/' + settings.id;
		}
...
FileUpload = {
	...
	upload: function (url, ctr, callBack) {
		...
		iframe.load(function () {
			...
			while (true) {
				try {
					data1 = iframe.contents().find("body").html();
					break;
				}
				catch (ex) { }
			}
...
Aux = {
	...
	***********************************************************************************/
	...
	setarFoco: function (container) {
		$('.setarFoco:enabled', container).first().focus();
	},
	...
	marcarCheck: function () {
		...
		if ($('input[type=checkbox]', linha).is(':checked')) {
