...
UnidadeProducaoItem = {
	...
	onAddResponsavelTecnico: function () {
		...
		$('tr:not(.trTemplate)', tabela).each(function () {
			...
			if (item.Id == responsavelTecnicoObj.Id) {
				itemAdicionado = true;
			}
		});
		...
		$('.hdnItemJson', linha).val(JSON.stringify(responsavelTecnicoObj));
		...
		$(".rbARTCargoFuncaoSim", UnidadeProducaoItem.container).click();
	...
	obter: function () {
		...
		$('.gridResponsaveis tbody tr:not(.trTemplate)', UnidadeProducaoItem.container).each(function () {
			unidadeProducaoItemObj.ResponsaveisTecnicos.push(JSON.parse($('.hdnItemJson', this).val()))
		});
	...
	salvar: function () {
		...
		$.ajax({
			...
			data: JSON.stringify({ unidade: unidade, empreendimentoID: UnidadeProducaoItem.settings.empreendimentoID }),
			cache: false,
			async: false,
			type: 'POST',
			dataType: 'json',
