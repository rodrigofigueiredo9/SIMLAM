...
Dominio = {
	...
	reservaAdicionar: function () {
		...
		var reserva = { EmpreendimentoId: +$('.hdnEmpreendimentoId', Dominios.container).val() };
		var dominioTipo = $('input[type="radio"]:checked', Dominio.container).val();
		var empInternoId = +$('.hdnEmpreendimentoInternoId', Dominios.container).val() == 0 ? -1 : +$('.hdnEmpreendimentoInternoId', Dominios.container).val();

		Modal.abrir(Dominio.settings.urls.reservaLegal, { reservaLegal: reserva, dominioTipo: dominioTipo, empInternoId: empInternoId }, function (container) {
	...
	reservaVisualizar: function () {
		...
		Modal.abrir(Dominio.settings.urls.reservaLegalVisualizar, { reservaLegal: reserva, empInternoId: empInternoId }, function (container) {
...
ReservaLegal = {
	...
	abrirModalEmpreendimento: function () {
		...
		var url = '';
		var empreendimentoId = +$('.hdnEmpreendimentoInternoId', Dominios.container).val() == 0 ? -1 : +$('.hdnEmpreendimentoInternoId', Dominios.container).val();
	...
	associarEmpreendimento: function (resposta) {
		...
		if (resposta.Id == +$('.hdnEmpreendimentoInternoId', Dominios.container).val()) {
		...
		$.ajax({
			...
			data: JSON.stringify({ empreendimentoInternoId: resposta.Id }),
			cache: false,
			async: false,
			type: 'POST',
			typeData: 'json',
		...
		if (sucesso && (+$('.ddlMatriculasCompensacao :selected', ReservaLegal.container).val() > 0) && $('.ddlARLIdentificacaoCompensacao', ReservaLegal.constructor).is(':visible')) {
			$('.ddlMatriculasCompensacao', ReservaLegal.container).change();
		}
	...
	carregarARLIdentificaoCompensacao: function () {
		...
		$.ajax({
			...
			data: JSON.stringify({ empreendimentoId: empreendimentoId, dominio: matriculaId }),
			cache: false,
			async: false,
			type: 'POST',
			typeData: 'json',
