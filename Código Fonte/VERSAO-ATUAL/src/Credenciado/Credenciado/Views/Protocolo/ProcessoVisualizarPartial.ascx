...
<div class="divModalProcesso divVisualizarProcesso">
			...
			<div class="block ultima prepend2">
				<span class="floatLeft inputFileDiv coluna82">
					...
					<% if(Model.Processo.Arquivo.Id.GetValueOrDefault() > 0) { %>
						<%= Html.ActionLink(ViewModelHelper.StringFit(Model.Processo.Arquivo.Nome, 43), "Baixar", "Protocolo", new { @id = Model.Processo.Arquivo.Id }, new { @Style = "display: block", @title = Model.Processo.Arquivo.Nome })%>
					<% } else { %>
	...
	<fieldset class="block box">
		...
		<div class="ultima prepend2">
			<span class="<%= (Model.Processo.ChecagemRoteiro.Id > 0) ? "" : "hide" %>">
				<a title="Visualizar checagem" class="icone esquerda inlineBotao visualizar btnVisualizarChecagem" href="#">Visualizar checagem</a>
			</span>
		</div>
	</fieldset>
	<% } %>
	...
	<fieldset class="block box">
		...
		<div class="block">
			...
			<div class="coluna30">
				<span class="<%= (Model.RequerimentoVM.Id > 0) ? "" : "hide" %>">
