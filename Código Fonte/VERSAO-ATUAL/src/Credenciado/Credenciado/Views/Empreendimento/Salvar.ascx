...
<div class="modalSalvarEmpreendimento">
	...
	<%= Html.Hidden("Empreendimento.IsCopiado", Model.IsCopiado, new { @class = "hdnIsCopiado" })%>

	<%if(Model.MostrarTituloTela){ %>
	<h1 class="titTela"><%= (Model.Empreendimento.Id > 0 || Model.Empreendimento.InternoId > 0) ? "Editar" : "Cadastrar"%> Empreendimento</h1>
	<%} %>
	<br />
	...
	<div class="block box">
		<div class="block">
			<div class="coluna30">
				...
				<%= Html.TextBox("Empreendimento.Codigo", Model.Empreendimento.Codigo > 0? Model.Empreendimento.Codigo.ToString() : "Gerado pelo IDAF", new { @class = "txtCodigo text disabled maskIntegerObrigatorio", @disabled="disabled"})%>
			</div>
			
			<%if(Model.Empreendimento.InternoId > 0){ %>
	...
	<fieldset class="divResponsaveis block box">
		...
		<br /><button class="btnAsmAdicionar direita">Responsável</button>
	</fieldset>
	...
	<fieldset class="block box">
		...
		<div class="block <%= Model.Empreendimento.TemCorrespondencia <= 0 ? "" : "hide" %>">
			<label>Utilizar o mesmo endereço de localização de empreendimento?</label>
			<label><%= Html.RadioButton("Empreendimento.TemCorrespondencia", IsCorrespondencia.NAO, Model.Empreendimento.TemCorrespondencia == IsCorrespondencia.NAO, new { @class = "radio rdbTemCorrespondencia" })%> Sim</label>
			<label class="append5"><%= Html.RadioButton("Empreendimento.TemCorrespondencia", IsCorrespondencia.SIM, Model.Empreendimento.TemCorrespondencia != IsCorrespondencia.NAO, new { @class = "radio rdbTemCorrespondencia" })%> Não</label>
		</div>
