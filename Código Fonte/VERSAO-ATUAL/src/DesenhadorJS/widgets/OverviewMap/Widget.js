  ...
  function(
    ...
    var clazz = declare([BaseWidget], {
      ...
      startup: function() {
		...
		var test = new ArcGISDynamicMapServiceLayer('http://mapas.geobases.es.gov.br/ArcGIS/rest/services/Vetor_Aerolevantamento/MapServer');
		this.map.addLayer(test);
				
		callWidgetFunction("OverviewMap", "onOpenDesenhador", "PROJETO = 111643");
	  ...
	  onOpenDesenhador: function(filtro) {
		var feat;
		var that = this;
		var entrada = {
		filtro: "PROJETO = 111643", 
		layers: [{
			nome: "ATP",
			url: "D_DOMINIALIDADE_NEW/FeatureServer/19",
			campos: ["PROJETO"]
		},{
			nome: "APMP",
			url: "D_DOMINIALIDADE_NEW/FeatureServer/18",
			campos: ["PROJETO"]
		}]};
		
		$.each(entrada.layers, function(i) {			
			feat = new FeatureLayer('https://webidafm/arcgis/rest/services/IDAF/' + entrada.layers[i].url 
			//+ '?token=jQCwtINVJzAM5xQAbB3zx36PoPY0JpGeuhoA_a8kihSLeSlNAp138vTeLRzHS1yd'
			, {
				id: entrada.layers[i].name,
				mode: FeatureLayer.MODE_ONDEMAND,
				outFields: entrada.layers[i].campos,
				//definitionExpression: entrada.filtro
			});
			that.map.addLayer(feat);			
		});
				
		queryTask = new QueryTask("https://webidafm/arcgis/rest/services/IDAF/D_DOMINIALIDADE_NEW/FeatureServer/18");
	    query = new Query();
	    query.returnExtentsOnly = true;
		query.where = entrada.filtro;
		queryTask.executeForExtent(query,showResults);
		
		function showResults(result) {
			//result.extent.spatialReference = that.map.spatialReference;
			that.map.setExtent(result.extent, true);
		}
