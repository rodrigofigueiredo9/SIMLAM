...
namespace Tecnomapas.EtramiteX.Scheduler.jobs
{
	public class CARSolicitacaoFunc
	{
		...
		public CARSolicita ObterInst(int id, OracleConnection conn, bool simplificado = false)//, BancoDeDados banco = null)
		{
			try
			{
		...
		internal List<CARSolicita> ObterSolicitacaoValida()
		{
			using (var conn = new OracleConnection(Class.CarUtils.GetBancoInstitucional()))
			{
				...
				using (OracleCommand command = new OracleCommand("" +
					@"select C.ID, c.situacao, cs.situacao_envio, 1 ORIGEM from TAB_CAR_SOLICITACAO c
						INNER JOIN TAB_CONTROLE_SICAR CS ON C.ID = CS.SOLICITACAO_CAR
						WHERE C.SITUACAO = 2 AND CS.SITUACAO_ENVIO = 6 AND C.ARQUIVO IS NULL
						UNION ALL
						select C.ID, c.situacao, cs.situacao_envio, 2 ORIGEM from IDAFCREDENCIADO.TAB_CAR_SOLICITACAO c
						INNER JOIN TAB_CONTROLE_SICAR CS ON C.ID = CS.SOLICITACAO_CAR
						WHERE C.SITUACAO = 2 AND CS.SITUACAO_ENVIO = 6 AND C.ARQUIVO IS NULL", conn))
				{
					using (var reader = command.ExecuteReader())
					{
						while (reader.Read())
						{
							CARSolicita item = new CARSolicita();
							item.Id = Convert.ToInt32(reader["id"]);
							item.SituacaoId = Convert.ToInt32(reader["situacao"]);
							item.SituacaoEnvioId = Convert.ToInt32(reader["situacao_envio"]);
							item.Origem = Convert.ToInt32(reader["origem"]);
		...
		internal void GerarPdfSolicitacaoCarInstitucional(CARSolicita solicitacao)
		{
			try
			{
				...
				_busRelatorio.SalvarPdfSolicitacaoCar(stream, dataSource);
			}
			catch (Exception ex)
			{
				Log.Error($"Erro ao gerar PDF CAR {solicitacao.Id} - Institucional: \\ {ex}");
			}
		}

		internal void GerarPdfSolicitacaoCarCredenciado(CARSolicita solicitacao)
		{
			try
			{
				RelatorioSolicitacao _busRelatorio = new RelatorioSolicitacao();
				CARSolicitacaoRelatorio dataSource = new CARSolicitacaoRelatorio();
				dataSource = _busRelatorio.ObterHistoricoCredenciado(solicitacao.Id);
				dataSource.DominialidadePDF = new DominialidadePDF(_busRelatorio.ObterCaracterizacaoCredenciado(dataSource.DominialidadeId, tid: dataSource.DominialidadeTid));

				MemoryStream stream = _busRelatorio.GerarPdf(dataSource, "~/Content/_pdfAspose/SolicitacaoInscricaoCAR_Credenciado.docx");
				_busRelatorio.SalvarPdfSolicitacaoCar(stream, dataSource);
			}
			catch (Exception ex)
			{
				Log.Error($"Erro ao gerar PDF CAR {solicitacao.Id} - Credenciado: \\ {ex}");
			}
		}
