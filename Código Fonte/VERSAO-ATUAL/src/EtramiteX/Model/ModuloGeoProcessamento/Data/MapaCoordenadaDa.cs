...
{
	public class MapaCoordenadaDa
	{
		private string EsquemaBanco { get; set; }
		private string EsquemaBancoGeo { get; set; }

		public MapaCoordenadaDa(string strBancoDeDados = null, string strBancoDeDadosGeo = null)
		{
		...
		public Lote ObterLote(string codfiscal, string codquadra, string codlote)
		{
			...
			{
				string queryStr = @"SELECT l.objectid,l.cod_sfisca,l.cod_quadra,l.cod_lote,l.nom_propri,l.tpo_lograd,l.nom_lograd,l.num_numera,l.nom_bairro 
		...
		private void ObterLoteXY(Lote lote)
		{
			...
			using (BancoDeDados bd = BancoDeDados.ObterInstancia())
			{
				string queryStr = "SELECT column_value ordenada FROM table (SELECT sdo_geom.sdo_mbr(t.geometry).sdo_ordinates FROM {0}lotes t WHERE t.objectid = :id AND rownum = 1)";
		...
		private void ObterLogradouroXY(Logradouro logradouro)
		{
			...
			using (BancoDeDados bd = BancoDeDados.ObterInstancia())
			{
				string queryStr = "SELECT column_value ordenada FROM table (SELECT sdo_geom.sdo_mbr(t.geometry).sdo_ordinates FROM {0}logradouros t WHERE t.objectid = :id AND rownum = 1)";
