...
{
	public class OficioAdministrativoDa
	{
		#region Propriedades
		...
		public OficioAdministrativoDa(string strBancoDeDados = null)
		{
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				select d.*, f.nome funcionario_nome, lm.texto municipio, le.texto estado, le.sigla uf,
					...
					from (select p.nome protocolo_nome,
								...
								p.assunto, (select fd.nome from tab_funcionario fd where fd.id = p.destinatario) destinatario, (select s.nome from tab_setor s where s.id = p.destinatario_setor) destinatario_setor, p.descricao, p.orgao_destino, p.cargo_destinatario, p.nome_destinatario, p.endereco_destinatario
				...
				using (IDataReader reader = bancoDeDados.ExecutarReader(comando))
				{
					if (reader.Read())
					{
						...
						protocolo.SetorDestinatario = string.Concat(reader.GetValue<String>("orgao_destino"), " - ", reader.GetValue<String>("cargo_destinatario"));
