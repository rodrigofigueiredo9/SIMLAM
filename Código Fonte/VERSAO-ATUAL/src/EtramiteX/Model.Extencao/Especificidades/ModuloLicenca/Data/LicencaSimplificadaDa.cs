...
{
	public class LicencaSimplificadaDa
	{
		#region Propriedades
		...
		public LicencaSimplificadaDa(string strBancoDeDados = null)
		{
		...
		internal void Salvar(LicencaSimplificada Licenca, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				Comando comando = bancoDeDados.CriarComando(@"select e.id from {0}esp_licenca_Simplificada e where e.titulo = :titulo", EsquemaBanco);
				comando.AdicionarParametroEntrada("titulo", Licenca.Titulo.Id, DbType.Int32);
				id = bancoDeDados.ExecutarScalar(comando);

				if (id != null && !Convert.IsDBNull(id))
				{
					comando = bancoDeDados.CriarComando(@"update {0}esp_licenca_Simplificada e set e.titulo = :titulo, e.protocolo = :protocolo, e.destinatario = :destinatario, e.barragem_id = :barragem_id, e.caracterizacao_id = (select distinct t.barragem from {0}crt_barragem_barragens t where t.id = :barragem_id), e.tid = :tid where e.titulo = 
                        :titulo", EsquemaBanco);
				...
				{
					comando = bancoDeDados.CriarComando(@"insert into {0}esp_licenca_Simplificada(id, titulo, protocolo, destinatario, tid, barragem_id, caracterizacao_id) values ({0}seq_esp_licenca_simplificada.nextval, :titulo, 
				...
				if (id == null || Convert.IsDBNull(id))
				{
					Licenca = Licenca ?? new LicencaSimplificada();
				...
				Historico.Gerar(Convert.ToInt32(Licenca.Titulo.Id), eHistoricoArtefatoEspecificidade.licencasimplificada, acao, bancoDeDados);
		...
		internal void Excluir(int titulo, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				Comando comando = bancoDeDados.CriarComando(@"update {0}esp_licenca_Simplificada c set c.tid = :tid where c.titulo = :titulo", EsquemaBanco);
				...
				Historico.Gerar(titulo, eHistoricoArtefatoEspecificidade.licencasimplificada, eHistoricoAcao.excluir, bancoDeDados);

				#endregion

				#region Apaga os dados da especificidade

				comando = bancoDeDados.CriarComando(@"delete from {0}esp_licenca_Simplificada e where e.titulo = :titulo", EsquemaBanco);
		...
		internal LicencaSimplificada Obter(int titulo, BancoDeDados banco = null)
		{
			LicencaSimplificada especificidade = new LicencaSimplificada();
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				(select distinct nvl(pe.nome, pe.razao_social) from {0}hst_esp_licenca_simplificada he, {0}hst_pessoa pe where he.destinatario_id = pe.pessoa_id and he.destinatario_tid = pe.tid
				...
				(select t.caracterizacao_tid from {0}hst_esp_licenca_simplificada t where t.id = (select max(id) from {0}hst_esp_licenca_simplificada where especificidade_id = e.id)) caracterizacao_tid 
				from {0}esp_licenca_Simplificada e, {0}tab_titulo_numero n, {0}tab_protocolo p where n.titulo(+) = e.titulo and e.protocolo = p.id(+) and e.titulo = :titulo", EsquemaBanco);
		...
		internal Licenca ObterDadosPDF(int titulo, BancoDeDados banco = null)
		{
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				Comando comando = bancoDeDados.CriarComando(@"select e.destinatario from {0}esp_licenca_Simplificada e where titulo = :id", EsquemaBanco);
