...
namespace Tecnomapas.EtramiteX.Interno.Model.Extensoes.Especificidades.ModuloLicenca.Data
{
	public class LicencaPreviaDa
	{
		...
		public LicencaPreviaDa(string strBancoDeDados = null)
		{
		...
		internal void Excluir(int titulo, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				Comando comando = bancoDeDados.CriarComando(@"update {0}esp_licenca_Previa c set c.tid = :tid where c.titulo = :titulo", EsquemaBanco);
				...
				Historico.Gerar(titulo, eHistoricoArtefatoEspecificidade.licencaprevia, eHistoricoAcao.excluir, bancoDeDados);

				#endregion

				#region Apaga os dados da especificidade

				comando = bancoDeDados.CriarComando(@"delete from {0}esp_licenca_Previa e where e.titulo = :titulo", EsquemaBanco);
		...
		internal LicencaPrevia Obter(int titulo, BancoDeDados banco = null)
		{
			LicencaPrevia especificidade = new LicencaPrevia();
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				(select distinct nvl(pe.nome, pe.razao_social) from {0}hst_esp_licenca_previa he, {0}hst_pessoa pe where he.destinatario_id = pe.pessoa_id and he.destinatario_tid = pe.tid
				...
				and he.tid = (select ht.tid from {0}hst_titulo ht where ht.titulo_id = e.titulo and ht.data_execucao = (select min(htt.data_execucao) from {0}hst_titulo htt where htt.titulo_id = e.titulo 
				...
				(select t.caracterizacao_tid from {0}hst_esp_licenca_previa t where t.id = (select max(id) from {0}hst_esp_licenca_previa where especificidade_id = e.id)) caracterizacao_tid 
				from {0}esp_licenca_Previa e, {0}tab_titulo_numero n, {0}tab_protocolo p where n.titulo(+) = e.titulo and e.protocolo = p.id(+) and e.titulo = :titulo", EsquemaBanco);
		...
		internal Licenca ObterDadosPDF(int titulo, BancoDeDados banco = null)
		{
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				Comando comando = bancoDeDados.CriarComando(@"select e.destinatario from {0}esp_licenca_Previa e where titulo = :id", EsquemaBanco);
