...
namespace Tecnomapas.EtramiteX.Interno.Model.Extensoes.Especificidades.ModuloEspecificidade.Business
{
	public class EspecificidadeBusBase
	{
		...
		internal static void CondicionantesConfigurarPdf(List<CondicionantePDF> condicionantes)
		{
			if (condicionantes != null && condicionantes.Count > 0)
			{
				condicionantes.ForEach(x =>
				{
					if (!String.IsNullOrEmpty(x.Prazo))
					{

						if (x.Periodo > 0)
						{
							String periodoTipo = string.Empty;
							if (x.Periodo > 1)
							{
								switch (x.PeriodoTipo)
								{
									case "Dia":
										periodoTipo = "dias";
										break;
							...
							{
								periodoTipo = x.PeriodoTipo.ToLower();
							}

							x.Descricao += String.Format(" - Prazo: {0} dias durante {1} {2}.", x.Prazo, x.Periodo, periodoTipo);
						}
						else
						{
							x.Descricao += String.Format(" - Prazo: {0} dias.", x.Prazo);
						}
					}
				});
			}
