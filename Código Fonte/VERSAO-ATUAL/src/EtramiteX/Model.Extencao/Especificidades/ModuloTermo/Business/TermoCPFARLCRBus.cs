...
{
	public class TermoCPFARLCRBus : EspecificidadeBusBase, IEspecificidadeBus
	{
		TermoCPFARLCRDa _da = new TermoCPFARLCRDa();
		...
		{
			get { return new TermoCPFARLCRValidar(); }
		}
		...
		{
			TermoCPFARLCR termo = especificidade as TermoCPFARLCR;
		...
		{
			return Deserialize(input, typeof(TermoCPFARLCR));
		}
		...
		public object ObterDadosPdf(IEspecificidade especificidade, BancoDeDados banco)
		{
			try
			{
				...
				termo.Titulo.TituloAnteriorMesEmissao = MesPorExtenso(termo.Titulo.TituloAnteriorMesEmissao);
