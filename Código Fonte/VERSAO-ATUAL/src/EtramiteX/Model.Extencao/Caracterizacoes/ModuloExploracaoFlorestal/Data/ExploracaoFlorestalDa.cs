	...
	class ExploracaoFlorestalDa
	{
		...
		internal int? Criar(ExploracaoFlorestal caracterizacao, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				comando.AdicionarParametroEntrada("tipo_exploracao", caracterizacao.TipoAtividade, DbType.Int32);
		...
		internal ExploracaoFlorestal ObterPorEmpreendimento(int empreendimento, bool simplificado = false, BancoDeDados banco = null)
		{
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				Comando comando = bancoDeDados.CriarComando(@"select s.id from {0}crt_exploracao_florestal s where s.empreendimento = :empreendimento and rownum = 1", EsquemaBanco);
				comando.AdicionarParametroEntrada("empreendimento", empreendimento, DbType.Int32);
		...
		internal ExploracaoFlorestal Obter(int id, BancoDeDados banco = null, bool simplificado = false)
		{
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				Comando comando = bancoDeDados.CriarComando(@"select c.empreendimento, c.codigo_exploracao, c.tipo_exploracao, c.data_cadastro, c.tid, lv.texto tipo_atividade from {0}crt_exploracao_florestal c
				...
				using (IDataReader reader = bancoDeDados.ExecutarReader(comando))
				{
					if (reader.Read())
					{
						...
						if (!Convert.IsDBNull(reader["tipo_atividade"]))
							caracterizacao.CodigoExploracaoTexto = reader["tipo_atividade"].ToString().Substring(0, 3) + caracterizacao.CodigoExploracao.ToString().PadLeft(3, '0');
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				using (IDataReader reader = bancoDeDados.ExecutarReader(comando))
				{
					...
					while (reader.Read())
					{
						...
						if (exploracaoAnterior.TipoAtividade != Convert.ToInt32(reader["tipo_exploracao"]))
						{
							...
							if (!Convert.IsDBNull(reader["tipo_exploracao"]))
								exploracao.TipoAtividade = Convert.ToInt32(reader["tipo_exploracao"]);
							if (!Convert.IsDBNull(reader["tipo_atividade"]))
								exploracao.CodigoExploracaoTexto = reader["tipo_atividade"].ToString().Substring(0, 3) + exploracao.CodigoExploracao.ToString().PadLeft(3, '0');
