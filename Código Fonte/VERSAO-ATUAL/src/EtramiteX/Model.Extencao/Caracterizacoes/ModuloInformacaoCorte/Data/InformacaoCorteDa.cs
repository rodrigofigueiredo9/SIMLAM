	...
	public class InformacaoCorteDa
	{
		...
		private void Editar(InformacaoCorte caracterizacao, BancoDeDados banco = null)
		{
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				Comando comando = bancoDeDados.CriarComando(@"update {0}crt_informacao_corte set empreendimento = :empreendimento, data_informacao = :data_informacao, area_flor_plantada = :area_flor_plantada, area_imovel = :area_imovel, tid = :tid where id = :id", EsquemaBanco);
		...
		internal List<InformacaoCorte> FiltrarPorEmpreendimento(int id, BancoDeDados banco = null, string esquema = null)
		{
			...
			using (BancoDeDados bancoDeDados = BancoDeDados.ObterInstancia(banco))
			{
				...
				from {0}crt_informacao_corte c where c.empreendimento = :id order by c.codigo", EsquemaBanco);
