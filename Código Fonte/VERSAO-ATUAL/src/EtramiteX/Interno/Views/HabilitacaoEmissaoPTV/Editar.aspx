		...
		$(function () {
			HabilitacaoEmissaoPTV.load($("#central"), {
				urls: {
					...
					salvar: '<%= Url.Action("Editar", "HabilitacaoEmissaoPTV") %>'
				},
				estadoDefaultId: <%= ViewModelHelper.EstadoDefaultId() %>,
				tiposArquivo: <%= Model.TiposArquivoValido %>,
				mensagens: <%= Model.Mensagens %>
			});
			$('.btnVerificarCpf').addClass('hide');
			$('#Pessoa_Fisica_Cpf').attr("disabled", true);
			$('#Pessoa_Fisica_Cpf').addClass("disabled");
		});
	</script>
</asp:Content>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
	<div id="central">
		<h1 class="titTela">Editar Habilitação para Emissão de PTV</h1>
		<br />
