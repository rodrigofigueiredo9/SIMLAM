...
<div class="divConteudoAtividade associarMultiplo">
	...
	<% bool mostrarColunaAcoes = (Model.IsEncerrar && Model.Atividades.Exists(x => x.SituacaoId != 6)) || Model.Atividades.Exists(x => x.SituacaoId == 6); %>
				...
				<table class="dataGridTable tabAtividades" width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					<%
						...
						<tr>
							...
							<%if (mostrarColunaAcoes) { %>
							<td>
								<%if (Model.IsEncerrar && atividade.SituacaoId != 6) { %>
									<button title="Encerrar atividade" class="icone dispensado btnEncerrarAtividade" value="" type="button"></button>
									<span class="spnVisualizarAtividade hide"><button title="Visualizar atividade" class="icone visualizar btnVisualizarAtividade" value="" type="button"></button></span>
								<%} else if(atividade.SituacaoId == 6) { %>
									<button title="Visualizar atividade" class="icone visualizar btnVisualizarAtividade" value="" type="button"></button>
								<%} %>
							</td>
							<%} %>
						</tr>
