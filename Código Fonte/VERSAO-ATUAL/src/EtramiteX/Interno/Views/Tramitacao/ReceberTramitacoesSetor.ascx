...
<table class="dataGridTable tabTramitacoesRecebidas" width="100%" border="0" cellspacing="0" cellpadding="0">
	<thead>
		<tr>
			<th><input type="checkbox" class="ckbCheckAllInMyColumn <%= Model.isRegistro ? "hide" : "" %>" <%= Model.ValidarCheckTodos(Model.TramitacoesSetor) ? "checked=\"checked\"": "" %> />Processo/Documento</th>
	...
	<tbody>
		<% for (int i = 0; i < Model.TramitacoesSetor.Itens.Count; i++) {
		 ...
		 bool selecionada = (Model.TramitacoesSetor.Itens[i].Id == Model.TramitacaoSelecionadaId);
		 %>
		<tr class="trTramitacao">
			...
			<td>
				<span class="trDataEnvio" title="<%= Html.Encode(tramitacao.DataEnvio.DataTexto) %>"><%= Html.Encode(tramitacao.DataEnvio.DataHoraTexto)%></span>
			</td>
			<td>
