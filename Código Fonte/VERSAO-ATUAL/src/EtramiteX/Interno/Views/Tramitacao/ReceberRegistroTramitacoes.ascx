<%@ Import Namespace="Tecnomapas.Blocos.Entities.Interno.ModuloTramitacao" %>
<%@ Import Namespace="Tecnomapas.EtramiteX.Interno.ViewModels.VMTramitacao" %>

<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<ReceberRegistroVM>" %>

<table class="dataGridTable tabTramitacoesRecebidas" width="100%" border="0" cellspacing="0" cellpadding="0">
	<thead>
		<tr>
			<th width="21%"><input type="checkbox" class="ckbCheckAllInMyColumn <%= Model.isRegistro ? "hide" : "" %>" />Processo/Documento</th>
			<th width="11%">Enviado em</th>
			<th width="9%">Origem</th>
			<th width="13%">Destinatário</th>
			<th width="10%">Motivo</th>
			<th width="10%">Ações</th>
		</tr>
	</thead>
	<tbody>
		<% for (int i = 0; i < Model.Tramitacoes.Itens.Count; i++) {
		  Tramitacao tramitacao = Model.Tramitacoes.Itens[i];
			string tramitacaoJson = Model.TramitacoesJson[i];
		%>
		<tr class="trTramitacao">
			<td>
				<input type="checkbox" class="ckbSelecionavel <%= Model.isRegistro ? "hide" : "" %>"  <%= Model.Tramitacoes.Itens[i].Id == Model.TramitacaoSelecionadaId ? "checked=\"checked\"" : ""%>/>
				<span class="trNumero iconeInline <%= (tramitacao.Protocolo.IsProcesso ? "processo" : "documento")%>" title="<%= Html.Encode(tramitacao.Protocolo.Numero) %>"><%= Html.Encode(tramitacao.Protocolo.Numero) %></span>
			</td>
			<td>
				<span class="trDataRecebido" title="<%= Html.Encode(tramitacao.DataEnvio.DataHoraTexto) %>"><%= Html.Encode(tramitacao.DataEnvio.DataHoraTexto)%></span>
			</td>
			<td>
				<span class="trSetorDestinatario" title="<%= Html.Encode(tramitacao.RemetenteSetor.Nome) %>"><%= Html.Encode(tramitacao.RemetenteSetor.Sigla)%></span>
			</td>
			<td>
				<span class="trDestinatario" title="<%= Html.Encode(tramitacao.Destinatario.Nome) %>"><%= Html.Encode(tramitacao.Destinatario.Nome)%></span>
			</td>
			<td>
				<span class="trObjetivo" title="<%= Html.Encode(tramitacao.Objetivo.Texto) %>"><%= Html.Encode(tramitacao.Objetivo.Texto)%></span>
			</td>
			<td>
				<input type="hidden" class="hdnTramitacaoId" value="<%= Html.Encode(tramitacao.Id) %>" />
				<input type="hidden" class="hdnTramitacaoDestinatarioId" value="<%= Html.Encode(tramitacao.Destinatario.Id) %>" />
				<input type="hidden" class="hdnTramitacaoJson" value="<%= Html.Encode(tramitacaoJson) %>" />
				<input type="hidden" class="hdnProtocoloId" value="<%= Html.Encode(tramitacao.Protocolo.Id) %>" />
				<input type="hidden" class="hdnIsProcesso" value="<%= Html.Encode(tramitacao.Protocolo.IsProcesso ? 1 : 2) %>" />
				<input type="button" title="Histórico de tramitação"  class="icone historico btnHistorico"/>
				<input type="button" title="PDF de despacho"  class="icone pdf btnPdf"/>
			</td>
		</tr>
		<%} %>
	</tbody>
</table>