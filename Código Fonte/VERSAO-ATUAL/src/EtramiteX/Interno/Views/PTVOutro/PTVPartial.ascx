...
<fieldset class="box">
	<div class="block">
		<div class="coluna20">
			...
			<label>
				<%= Html.RadioButton("NumeroTipo", (int)eDocumentoFitossanitarioTipoNumero.Digital, Model.PTV.NumeroTipo == (int)eDocumentoFitossanitarioTipoNumero.Digital, ViewModelHelper.SetaDisabled(Model.PTV.Id > 0, new { @class="rbTipoNumero rbTipoNumeroDigital"}))%>Nº Digital
			</label>
		</div>
		<div class="coluna22">
		...
		<div class="coluna15">
			...
			<%=Html.TextBox("DataEmissao", !string.IsNullOrEmpty(Model.PTV.DataEmissao.DataTexto)?Model.PTV.DataEmissao.DataTexto:DateTime.Today.ToShortDateString(), ViewModelHelper.SetaDisabled(Model.PTV.Id >0 || Model.PTV.NumeroTipo != (int)eDocumentoFitossanitarioTipoNumero.Digital || Model.IsVisualizar, new { @class="text txtDataEmissao maskData"}))%>
		</div>
		<div class="coluna14">
	...
	<div class="block campoTela <%= Model.PTV.Id <= 0 ? "hide":""%>">
		...
		<div class="campoTela <%= Model.PTV.Id <= 0 ? "" : "hide" %>">
			<div class="coluna10">
				<button type="button" class="inlineBotao btnAssociarEmpreendimento">Buscar</button>
			</div>
		</div>
	</div>
...
<fieldset class="block box identificacao_produto campoTela  <%= Model.PTV.Id <= 0 ? "hide":""%>">
	...
	<div class="block">
		...
		<div class="coluna22 campoIdentificacao">
			<label for="NumeroOrigem">Número Origem *</label>
			<%=Html.DropDownList("NumeroOrigem",  new List<SelectListItem>(), ViewModelHelper.SetaDisabled(Model.IsVisualizar, new { @class="text ddlNumeroOrigem campoIdentificacao1"}))%>
			<%=Html.TextBox("NumeroOrigem",  (object)String.Empty, ViewModelHelper.SetaDisabled(Model.IsVisualizar, new { @class="text txtNumeroOrigem maskNumInt campoIdentificacao2 hide",  @maxlength="20"})) %>
		</div>
		<div class="coluna20">
		...
		<div class="coluna10 campoIdentificacao2 hide">
...
<div class="block box campoTela <%= Model.PTV.Id <= 0 ? "hide":""%>">
	<div class="block">
		<div class="coluna25">
			...
			<label>
				<%=Html.RadioButton("PartidaLacradaOrigem", (int)ePartidaLacradaOrigem.Nao, Model.PTV.PartidaLacradaOrigem == (int)ePartidaLacradaOrigem.Nao , ViewModelHelper.SetaDisabled(Model.IsVisualizar, new { @class="rbPartidaLacradaOrigem rbLacradaOrigemNao"}))%>
				Não
			</label>
		</div>
		<div class="partida_lacrada <%= Model.PTV.PartidaLacradaOrigem == (int)ePartidaLacradaOrigem.Sim ?"":"hide" %>">
			<div class="coluna15">
	...
	<div class="block">
		<div class="coluna20">
			...
			<label for="PossuiLaudoLaboratorial">
				<%=Html.RadioButton("PossuiLaudoLaboratorial", (int)eLaudoLaboratorial.Sim, Model.PTV.PossuiLaudoLaboratorial == (int)eLaudoLaboratorial.Sim, ViewModelHelper.SetaDisabled(Model.IsVisualizar, new { @class="rbPossuiLaudo rbPossuiLaudoSim" }))%>
				Sim
			</label>
			<label>
				<%=Html.RadioButton("PossuiLaudoLaboratorial", (int)eLaudoLaboratorial.Nao, Model.PTV.PossuiLaudoLaboratorial == (int)eLaudoLaboratorial.Nao, ViewModelHelper.SetaDisabled(Model.IsVisualizar, new { @class="rbPossuiLaudo rbPossuiLaudoNao" }))%>
				Não
			</label>
		</div>
		<div class="coluna75 prepend1 laudo <%= Model.PTV.PossuiLaudoLaboratorial == (int)eLaudoLaboratorial.Sim ?"":"hide" %>">
			<label for="LaboratorioNome">Nome do laboratório *</label>
			<%=Html.DropDownList("LaboratorioNome", Model.LstLaboratorio, ViewModelHelper.SetaDisabled(Model.IsVisualizar, new { @class="text ddlNomeLaboratorio" }))%>
		</div>
	</div>
	<div class="block divEndereco laudo <%= Model.PTV.PossuiLaudoLaboratorial == (int)eLaudoLaboratorial.Sim ? "":"hide"%>">
		<div class="coluna45">
			<label for="LaudoResultadoAnalise">Número do laudo com resultado da análise *</label>
			<%=Html.TextBox("LaudoResultadoAnalise", Model.PTV.LaudoResultadoAnalise, ViewModelHelper.SetaDisabled(true, new { @class="text txtLaudoResultadoAnalise"}))%>
		</div>
		<div class="coluna18 prepend1">
			<label for="EstadoId">UF *</label>
			<%=Html.DropDownList("EstadoId", Model.Estados, ViewModelHelper.SetaDisabled(true, new { @class="text ddlEstados"}))%>
		</div>
		<div class="coluna30 prepend1">
			<label for="MunicipioId">Municipio *</label>
			<%=Html.DropDownList("MunicipioId", Model.MunicipioLaboratorio, ViewModelHelper.SetaDisabled(true, new { @class="text ddlMunicipios"}))%>
		</div>
	</div>
</div>
...
<div class="block box campoTela <%= Model.PTV.Id <= 0 ? "hide":""%>">
	...
	<div class="block">
		<div class="coluna24">
			...
			<label>
				<%= Html.RadioButton("RotaTransitoDefinida", (int)eRotaTransitoDefinida.Sim, Model.PTV.RotaTransitoDefinida == (int)eRotaTransitoDefinida.Sim ,ViewModelHelper.SetaDisabled(Model.IsVisualizar, new { @class="rdbRotaTransitoDefinida rdbRotaTransitoDefinidaSim"}))%>Sim				
			</label>
			<label>
		...
		<div class="coluna36 rota <%= Model.PTV.RotaTransitoDefinida == null||Model.PTV.RotaTransitoDefinida <= 0 ? "hide":"" %>">
	...
	<div class="block">
		<div class="coluna24">
			...
			<label>
				<%=Html.RadioButton("NotaFiscalApresentacao", (int)eApresentacaoNotaFiscal.Sim, Model.PTV.NotaFiscalApresentacao == (int)eApresentacaoNotaFiscal.Sim, ViewModelHelper.SetaDisabled(Model.IsVisualizar, new { @class="rdbApresentacaoNotaFiscal rdbApresentacaoNotaFiscalSim" }))%>
				Sim
			</label>
			<label>
		...
		<div class="coluna36 nota_fical <%=Model.PTV.NotaFiscalApresentacao == null||Model.PTV.NotaFiscalApresentacao <= 0? "hide":"" %>">
			<label for="NotaFiscalNumero">Nº da nota fiscal *</label>
			<%= Html.TextBox("NotaFiscalNumero", Model.PTV.NotaFiscalNumero, ViewModelHelper.SetaDisabled(Model.IsVisualizar , new { @class="text txtNotaFiscalNumero", @maxlength="15" })) %>
		</div>
	</div>
</div>
...
<div class="block box campoTela <%= Model.PTV.Id <= 0 ? "hide":""%>">
	...
	<div class="block">
		<div class="coluna40">
			...
			<%=Html.DropDownList("ResponsavelTecnico", Model.LsResponsavelTecnico, ViewModelHelper.SetaDisabled(Model.IsVisualizar, new { @class="text ddlResponsavelTecnico" }))%>
		</div>
	</div>
	<div class="block">
		<div class="coluna40">
