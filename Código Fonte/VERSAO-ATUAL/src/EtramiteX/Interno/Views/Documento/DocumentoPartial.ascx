...
<div class="divModalDocumento">
    ...
    <div class="block box">
        ...
        <div class="block">
            ...
            <div class="qtdDocumento coluna18  <%= Model.Tipo.PossuiQuantidadeDocumento || Model.Tipo.QuantidadeDocumentoObrigatorio ? "" : "hide" %>">
                <label class="lblQtdDocumento">Qtd. de documento <%: Model.Asterisco(Model.Tipo.QuantidadeDocumentoObrigatorio) %></label>
            ...
            <div class="block ultima prepend2">
                <span class="floatLeft inputFileDiv coluna82 <%= Model.Tipo.PossuiAssunto || Model.Tipo.AssuntoObrigatorio ? "hide" : "" %>">
                    ...
                    <span class="spanInputFile <%= string.IsNullOrEmpty(Model.Documento.Arquivo.Nome) ? "" : "hide" %>">
                        <input type="file" class="inputFile" style="display: block; width: 100%" name="file" /></span>
                    <input type="hidden" class="hdnArquivoJson" value="<%: Model.ObterJSon(Model.Documento.Arquivo) %>" />
                </span>

                <span class="spanBotoes prepend2  <%= Model.Tipo.PossuiAssunto || Model.Tipo.AssuntoObrigatorio ? "hide" : "" %>">
        ...
        <div class="block">
            <div class="nomeDocumento coluna86 <%= Model.Tipo.PossuiNome || Model.Tipo.NomeObrigatorio  ? "" : "hide" %>">
                <label class="lblNome">Nome do documento <%: Model.Asterisco(Model.Tipo.NomeObrigatorio) %></label>
            ...
            <div class="descricao coluna86 <%= Model.Tipo.PossuiDescricao || Model.Tipo.DescricaoObrigatoria ? "" : "hide" %>">
                ...
                <%= Html.TextArea("Documento.Descricao", Model.Documento.Descricao, new { @class = "text txtDescricao" })%>
            </div>
            <div class="destinatarioLivre coluna86 <%= Model.Tipo.PossuiDestinatarioLivre || Model.Tipo.DestinatarioLivreObrigatorio ? "" : "hide" %>">
                ...
                <%= Html.TextArea("Documento.Descricao", Model.Documento.Descricao, new { @class = "text txtDestinatarioLivre" })%>
            </div>
        </div>
    </div>
    ...
    <fieldset class="containerProcessoDocumento block box <%= Model.ExibirGrupoProtocolo ? "" : "hide" %>">
        ...
        <div class="coluna60">
            <label>
                <%= Html.RadioButton("ProtocoloAssociadoTipo", 1, (Model.RadioProcessoChecked), new { @class = "radio rdbProtocoloAssociadoTipo visivelSeAssociaDocumento " + (Model.ExibirRadioProcDoc ? "" : "hide") })%> Nº de registro de
                <label class="lblProcesso">processo<%: Model.Asterisco(Model.Tipo.ProcessoObrigatorio) %></label></label>
            <label class="visivelSeAssociaDocumento <%= (Model.ExibirRadioProcDoc) ? "" : "hide" %>">
                <%= Html.RadioButton("ProtocoloAssociadoTipo", 2, Model.RadioDocumentoChecked, new { @class = "radio rdbProtocoloAssociadoTipo" })%> Nº de registro de
                <label class="lblDocumento">documento<%: Model.Asterisco(Model.Tipo.DocumentoObrigatorio) %></label></label>
    ...
    <fieldset class="containerChecagemPendencia block box <%= (Model.Tipo.PossuiChecagemPendencia || Model.Tipo.ChecagemPendenciaObrigatorio) ? "" : "hide" %>">
        ...
        <div class="coluna30 prepend1">
            <% if (!Model.IsEditar)
				{ %>
            <button type="button" title="Buscar checagem" class="floatLeft inlineBotao botaoBuscar btnBuscarChecagemPendencia">Buscar</button><% } %>
    ...
    <fieldset class="destinatario block box <%= (Model.Tipo.PossuiAssunto || Model.Tipo.AssuntoObrigatorio) && !(Model.Tipo.PossuiDestinatarioLivre || Model.Tipo.DestinatarioLivreObrigatorio)  ? "" : "hide" %>">
        ...
        <div class="block divDropDown">
            <div class="coluna48">
                ...
                <%= Html.DropDownList("Documento.DestinatarioSetor.Id", Model.SetoresDestinatario, new { @class = "text ddlSetoresDestinatario" })%>
            </div>
            <div class="coluna45 prepend2 ddlFuncionario">
                <label for="Enviar_Destinatario_Id">Funcionário</label>
                <%= Html.DropDownList("Documento.Destinatario.Id", Model.DestinatarioFuncionarios, ViewModelHelper.SetaDisabled(Model.Documento.Destinatario.Id == 0, new { @class = "text ddlDestinatarios" }))%>
            </div>
        </div>
    </fieldset>
    ...
    <fieldset class="destinatarioLivreFieldSet block box <%= Model.Tipo.PossuiDestinatarioLivre || Model.Tipo.DestinatarioLivreObrigatorio ? "" : "hide" %>">
        ...
        <div class="block divDropDown">
            <div class="coluna50">
                ...
                <%= Html.TextBox("Documento.OrgaoDestino", Model.Documento.OrgaoDestino, new { @class = "text txtOrgaoDestino", @maxlength = 100 })%>
            </div>
			<div class="coluna33 prepend2">
                <label for="Enviar_CargoFuncaoDestinatario">Cargo / Função Destinatário *</label>
                <%= Html.TextBox("Documento.CargoFuncaoDestinatario", Model.Documento.CargoFuncaoDestinatario, new { @class = "text txtCargoFuncaoDestinatario", @maxlength = 100 })%>
            </div>
			<div class="coluna70">
                <label for="Enviar_NomeDestinatario">Nome Destinatário *</label>
                <%= Html.TextBox("Documento.NomeDestinatario", Model.Documento.NomeDestinatario, new { @class = "text txtNomeDestinatario", @maxlength = 100 })%>
            </div>
			<div class="coluna70">
                <label for="Enviar_EnderecoDestinatario">Endereço Destinatário *</label>
                <%= Html.TextBox("Documento.EnderecoDestinatario", Model.Documento.EnderecoDestinatario, new { @class = "text txtEnderecoDestinatario", @maxlength = 100 })%>
            </div>
        </div>
    </fieldset>
