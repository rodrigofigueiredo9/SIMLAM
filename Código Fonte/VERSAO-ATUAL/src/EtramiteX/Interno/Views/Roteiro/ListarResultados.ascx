	...
	<table class="dataGridTable ordenavel" width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<% foreach (var item in Model.Resultados) { %>
			<tr>
				...
				<td class="roteiroSetorSigla" title="<%= Html.Encode(item.SetorSigla)%>"><%= Html.Encode(item.SetorSigla)%></td>
				...
				<td>
					...
					<%if (Model.MostrarRelatorio) { %><input type="button" title="PDF do Roteiro" class="icone pdf btnPdf" /><%}%>
					<%if (Model.PodeDesativar) {%><input type="button" title="Desativar" class="icone cancelar btnDesativar" /><%}%>
				</td>
			</tr>
		<% } %>
		</tbody>
