...
<asp:Content ID="Content2" ContentPlaceHolderID="JsHeadContent" runat="server">
	<script type="text/javascript" src="<%= Url.Content("~/Scripts/Areas/Caracterizacoes/regularizacaoFundiaria.js") %>"></script>
		...
		$(function () {
			RegularizacaoFundiaria.load($('#central'), {
				urls: {
					...
					pessoaVisualizar: '<%= Url.Action("PessoaModalVisualizar", "Pessoa", new {area =""}) %>',
					...
					salvar: '<%= Url.Action("Editar", "RegularizacaoFundiaria") %>'
				},
...
<asp:Content ID="Content3" ContentPlaceHolderID="MainContent" runat="server">	
	<div id="central">
		...
		<% bool esconderSalvar = Model.Caracterizacao.Posses.Exists(x => x.AreaRequerida <= 0); %>
		<div class="block box">
			<input class="floatLeft btnSalvar <%= esconderSalvar ? "hide" : "" %>" type="button" value="Salvar" />
			<span class="cancelarCaixa cancelarCaixaPrincipal"><span class="btnModalOu <%= esconderSalvar ? "hide" : "" %>">ou</span> <a class="linkCancelar" href="<%= Url.Action("", "Caracterizacao", new { id = Model.Caracterizacao.EmpreendimentoId}) %>">Cancelar</a></span>
		</div>
	</div>
</asp:Content>