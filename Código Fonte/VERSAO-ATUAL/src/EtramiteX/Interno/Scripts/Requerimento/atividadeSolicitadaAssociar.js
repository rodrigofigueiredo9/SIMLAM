...
FinalidadeAssociar = {
	...
	onAssociarFinalidade: function () {
		...
		if (!$('.modeloTituloConteudo', FinalidadeAssociar.container).hasClass('hide')) {
			...
			if ($('.emitidoPorInterno', FinalidadeAssociar.container).attr('checked')) {
	...
	limparCamposTituloAnterior: function () {
		...
		$('.emitidoPorInterno', FinalidadeAssociar.container).trigger('change');
	...
	carregarDropDown: function (valores, dropDown, textoPadrao) {
		...
		dropDown.removeClass('disabled').removeAttr('disabled', 'disabled').find('option:first').attr('selected', 'selected');
	}
}