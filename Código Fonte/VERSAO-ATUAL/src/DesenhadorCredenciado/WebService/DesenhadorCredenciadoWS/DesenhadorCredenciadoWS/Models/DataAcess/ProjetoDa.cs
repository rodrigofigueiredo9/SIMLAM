	...
	public class ProjetoDa
	{
		...
		internal bool VerficiarProjetoFinalizado(int idProjeto, bool isFiscalizcao=false)
		{
			...
			try
			{
				...
				comando = bancoDeDadosGeo.GetComandoSql(@"select count(*) from tmp_projeto_geo c where c.id = :projeto");
				comando.AdicionarParametroEntrada("projeto", idProjeto, DbType.Int32);

				if (Convert.ToInt32(bancoDeDadosGeo.ExecutarScalar(comando)) > 0)
				{
					return false;
				}
				...
				if (!isFiscalizcao)
				{
					comando = bancoDeDadosGeo.GetComandoSql(@"select count(*) from crt_projeto_geo c where c.id = :projeto");
					comando.AdicionarParametroEntrada("projeto", idProjeto, DbType.Int32);
				...
				/*else
				{
					comando = bancoDeDadosGeo.GetComandoSql(@"select count(*) from tab_fisc_prj_geo c where c.id = :projeto");
					comando.AdicionarParametroEntrada("projeto", idProjeto, DbType.Int32);
		...
		internal bool VerficiarProjetoCARFinalizado(int idProjeto)
		{
			...
			try
			{
				...
				comando = bancoDeDadosGeo.GetComandoSql(@"select count(*) from tmp_cad_ambiental_rural c where c.projeto_geo_id = :projeto");
				comando.AdicionarParametroEntrada("projeto", idProjeto, DbType.Int32);
				...
				comando = bancoDeDadosGeo.GetComandoSql(@"select count(*) from crt_cad_ambiental_rural c where c.projeto_geo_id = :projeto");
				comando.AdicionarParametroEntrada("projeto", idProjeto, DbType.Int32);
		...
		*/
        ...
        internal Projeto BuscarDadosProjeto(int idProjeto, int idFilaTipo)
        {
            ...
            try
            {
                ...
                {
						 comando = bancoDeDadosGeo.GetComandoSql(@"select (case when a.caracterizacao =1 then 1 else 2 end ) navegador, a.empreendimento from crt_projeto_geo a where a.id = :projeto
					union all
