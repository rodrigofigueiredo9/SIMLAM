	...
	<table class="dataGridTable ordenavel" width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<% foreach (var item in Model.Resultados) { %>
			<tr>
				...
				<td title="<%= Html.Encode(item.Tipo)%>"><%= Html.Encode(item.Tipo)%></td>
				<td title="<%= Html.Encode(item.SituacaoTexto)%>"><%= Html.Encode(item.SituacaoTexto)%></td>
				<td>
					<input type="hidden" class="itemJson" value="<%: ViewModelHelper.Json(new { Id = item.ID, NumeroTipo = item.NumeroTipo , Numero = item.Numero, CulturaCultivar = item.CulturaCultivar, SituacaoTexto = item.SituacaoTexto, Tipo = item.Tipo, Situacao = item.Situacao } ) %>" />
