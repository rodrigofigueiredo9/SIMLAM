...
MasterPage = {
	...
	executarAjax: function (url, params, container, async) {
		var objResponse = null;
		var ehModal = MasterPage.getContent(container).hasClass('modalContent');
		...
		$.ajax({ url: url, data: JSON.stringify(params), cache: false, async: async,
			type: 'POST', dataType: 'json', contentType: 'application/json; charset=utf-8',
			error: function (XMLHttpRequest, textStatus, erroThrown) {
				var arrayMensagem = Aux.error(XMLHttpRequest, textStatus, erroThrown, MasterPage.getContent(container));
				objResponse = { EhValido: false, Msg: arrayMensagem };
			},
			success: function (response, textStatus, XMLHttpRequest) {
				Mensagem.limpar(MasterPage.getContent(container));
				objResponse = response;
...
Mascara = {
	...
	load: function (container) {
		...
		//Coordenada GMS
		$.mask.definitions['~'] = '[-+]';
		$(".maskCoordGms", container).mask("~99:99:99,99");

		//Coordenada GDEC
		$(".maskCoordGdec", container).mask("~99,9999999", { placeholder: "0" });

		//Coordenada UTM
		$(".maskUtm", container).mask("9999999,99999");

		// CEP
		$(".maskCep", container).mask("99.999-999");

		//CCIR - Certificado de Cadastro de Imovel Rural
		$(".maskCcir", container).mask("999.999.999.999-9");

		//NIRF - Número do Imóvel na Receita Federal
		$(".maskNirf", container).mask("9.999.999-9");

		//FONE - Número Telefonico
		$(".maskFone", container).mask("(99) 9999-9999");

		//CPF - Cadastro de Pessoa Física
		$(".maskCpf", container).mask("999.999.999-99");

		//CPF - Cadastro de Pessoa Física
		$(".maskCpfParcial", container).mask("999.999.999-99");

		//CNPJ - Cadastro Nacional de Pessoa Juridica
		$(".maskCnpj", container).mask("99.999.999/9999-99");

		//CNPJ - Cadastro Nacional de Pessoa Juridica
		$(".maskCnpjParcial", container).mask("99.999.999/9999-99");

		//DATA - Campo de data padrão
		$(".maskData", container).mask("39/19/9999");

		//Número da caixa do arquivo (3 dig.)
		$(".maskNum3", container).mask("999");

		//Número da caixa do arquivo (4 dig.)
		$(".maskNum4", container).mask("9999");

		$(".maskNum15", container).mask("999999999999999");

		//Número do Endereço
		$(".maskNumEndereco", container).mask("999999");

		//Área Abrangencia
		$(".maskAreaAbrangencia", container).mask("9999999,99");

		//Documento Anterior
		$(".maskDocumentoAnterior", container).mask("9999999/9999");

		//Máscara de processo
		$(".maskProc", container).mask("9999999/9999");

		//Quantidade Volumes Processo
		$(".maskQuantidadeVolumes", container).mask("99");

		// Mascara 5,4 para campos de área
		$(".maskArea54", container).mask("99999,9999");

		// Mascara 7,4 para campos de área
		$(".maskArea74", container).mask("9999999,9999");

		// Mascara de hora
		$(".maskHora", container).mask("99:99:99");

		$(".maskDecimal", container).maskMoney();

		$(".maskNumInt", container).each(function (idx, item) {
			$(item).mask(Mascara.criarMascara($(item).attr('maxlength')), 1);
		});

		$(".maskString", container).each(function (idx, item) {
			$(item).mask(Mascara.criarMascara($(item).attr('maxlength')), 2);
		});
