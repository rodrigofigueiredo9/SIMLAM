...
namespace Tecnomapas.EtramiteX.Configuracao.Interno.Extensoes.Data
{
	class ListaValoresDa
	{
		...
		internal List<Lista> ObterCaracterizacaoProdutosInformacaoCorte()
		{
			List<Lista> lst = new List<Lista>();
			IEnumerable<IDataReader> daReader = DaHelper.ObterLista(@"select t.id, t.texto from lov_crt_produto_inf_corte t");
			foreach (var item in daReader)
			{
				lst.Add(new Lista()
				{
