...
namespace Tecnomapas.Blocos.Etx.ModuloValidacao
{
	...
	{
		private static TermoCPFARLCRMsg _termoCPFARLCRMsg = new TermoCPFARLCRMsg();
		public static TermoCPFARLCRMsg TermoCPFARLCR
		{
			get { return _termoCPFARLCRMsg; }
			set { _termoCPFARLCRMsg = value; }
		}
	}

	public class TermoCPFARLCRMsg
	{
		...
		public Mensagem NumeroAverbacaoObrigatorio { get { return new Mensagem() { Tipo = eTipoMensagem.Advertencia, Campo = "NumeroAverbacao", Texto = "Nº da averbação é obrigatório." }; } }
		...
		{
			return new Mensagem() { Tipo = eTipoMensagem.Advertencia, Campo = "CedenteDominio, #ReceptorDominio", Texto = String.Format("A Matrícula/Posse do empreendimento receptor já está associada a outro Título de \"Termo de Compromisso de Preservação e/ou Formação de Área de Reserva Legal por Compensação - Retificação\" na situação \"{0}\" para a mesma matrícula ou posse do empreendimento cedente.", situacao) };
		}
